...
const DESTINATIONS: Destination[] = [
  ...
  },
  {
    ...
    text: ({ one }) =>
      ...
      + '6) 초안은 **포털 토큰 페이지에서 지정한 RA 워크스페이스**에 생깁니다(기본은 소속 조직). '
      + '만든 뒤 어느 워크스페이스에 생겼는지 확인해 알려 주세요 — 개인함(personal-…)이면 '
      + '남이 못 보니, 포털 “내 연결”에서 조직을 다시 고르라고 일러 주세요.\n'
      + '7) 워크스페이스와 **게시판은 다릅니다.** 부서 게시판에 걸려면 list_boards 로 후보를 '
      + '보여 주고, 내가 고르면 preview_publish 로 대상을 확인시킨 뒤 publish_report 로 '
      + '게시하세요(확인 토큰 10분·그 게시판 조합에만 유효). 폴더는 list_folders → '
      + 'set_report_folders. 게시하지 말라고 하면 초안으로 두세요.',
  },
  {
    key: 'wiki',
    label: '📘 MX 백서에 싣기',
    ...
    <div className="doc-actions" role="group" aria-label="붙인 문서를 어디로 보낼지">
      ...
      {can('plat:reportarchive') && (
        <details className="doc-actions-note">
          ...
          <br />
          보고서가 쌓이는 <b>워크스페이스</b>는 포털 “내 연결”에서 RA 토큰을 등록할 때 정해집니다
          (기본은 소속 조직). 개인함으로 돼 있으면 남이 못 보니 거기서 바꾸세요.
        </details>
      )}
    </div>
  );
