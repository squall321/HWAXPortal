...
export async function streamChat(
  ...
  opts: {
    ...
    pinnedAgent?: string;
    /** 표시용 이름 — 서버 대화에 같이 적어 두었다가 다른 기기에서 되살린다. */
  ...
  const { systemId, mode, history, conversationId, delibOpts, pinnedTools, pinnedApps, pinnedAgent, pinnedAgentName, pinnedAgents, searchSources, thinking, documents, signal, ...handlers } = opts;
  const csrf = getCookie('hwax_csrf');
  ...
  const res = await apiFetch(`/agent/chat${qs}`, {
    ...
    body: JSON.stringify({
      ...
      // 키가 아니라 사람이 읽을 이름으로 되살아나게. agent-server 로는 안 넘어간다.
      ...(pinnedAgent && pinnedAgentName ? { pinned_agent_name: pinnedAgentName } : {}),
