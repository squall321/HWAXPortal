"""워크벤치용 단명 PAT — **단계 호출마다** 찍고 런에 저장하지 않는다.
...
1. **창(window) 결정성을 안 쓴다.** 그것은 agent-server 의 에이전트 캐시가 첫 토큰을 물고
   재사용하기 때문에 필요했다. 워크벤치는 agent-server 를 안 쓰고 게이트웨이는 요청마다
   검증하므로 요건이 아니다(따라도 무해하지만, 안 따르는 편이 수명이 길다).
2. **수명이 짧다.** 찍자마자 그 호출에만 쓴다. `_chat_user_pat` 은 창 시작+60분이라 잔여
   수명이 30~60분인데, 런에 저장했다가 `gate: human` 뒤에 재개하면 401 이 난다(실측).
3. **`pat_name`·`jti` 로 챗과 구분한다.** 게이트웨이 감사는 이 클레임을 안 남기므로
   구분은 워크벤치 자기 런 기록이 맡지만(context-notes W-17), 토큰 자체에도 적어 둔다 —
...
def mint(keystore, settings, principal, run_id: str, step_ix: int) -> str | None:
    ...
    챗은 발급에 실패해도 GW_TOKEN 으로 도는 길이 있지만(대화가 끊기지 않게), 워크벤치는
    그러면 안 된다. 런 기록의 명의가 실제 호출자와 달라지면 그 기록은 감사 정본이 못 된다.
    """
    try:
