"""워크벤치 API — prefix `/workbench-api`.

접두사가 SPA 경로 `/workbench/*` 와 다른 이유 — 같으면 새로고침이 JSON 을 받는다.
`App.tsx` 가 `/changelog`(API)와 `/updates`(SPA)를 가른 것과 같은 사고를 피한다.

**모든 라우트가 `ensure(principal, "feat:workbench")` 를 부른다.** `access.yaml` 의
`features:` 선언만으로는 아무 라우트도 안 막힌다 — 기능 키는 타일·게이트웨이 백엔드에만
자동으로 묶인다. 실행 계열은 거기에 더해 런 소유자까지 본다.

실행은 **요청 밖에서** 돈다(202 + 폴링). 워크벤치 경로는 nginx catch-all 이라 기본
...
from app.workbench.runner import RunnerError

logger = logging.getLogger(__name__)
router = APIRouter(prefix="/workbench-api", tags=["workbench"])

FEAT = "feat:workbench"
# create_task 의 결과를 붙잡아 둔다 — 안 잡으면 GC 가 실행 중인 런을 거둬 간다.
_TASKS: set[asyncio.Task] = set()


def _store(request: Request):
    st = getattr(request.app.state, "workbench_store", None)
    if st is None:
        raise AuthError("워크벤치 저장소가 열리지 않았습니다", status_code=503)
    return st


def _runner(request: Request):
    r = getattr(request.app.state, "workbench_runner", None)
    if r is None:
        raise AuthError("워크벤치 실행기가 서지 않았습니다", status_code=503)
    return r
...
def _owned(request: Request, principal: Principal, run_id: str) -> dict:
    """런은 공유하지 않는다 — 결과에 그 사람 시야의 데이터가 담긴다."""
    run = _store(request).get_run(run_id, owner_sub=principal.subject)
    if run is None:
        raise AuthError("런을 찾을 수 없습니다", status_code=404)
    return run
...
def _spawn(coro, what: str) -> None:
    ...
    def _done(task: asyncio.Task) -> None:
        ...
        if not task.cancelled() and task.exception() is not None:
            logger.error("워크벤치 %s 실패", what, exc_info=task.exception())
...
def health(request: Request, response: Response) -> dict:
    ...
    if st is None:
        ...
        return {"ok": False,
                "error": getattr(request.app.state, "workbench_error", "not initialised")}
    try:
...
async def _tools_map(request: Request) -> dict:
    url = (getattr(request.app.state, "workbench_runner").gateway_url or "").rstrip("/")
    try:
...
def _validated(request: Request, raw: dict) -> tuple[RecipeSpec, list[str]]:
    spec = RecipeSpec.model_validate(raw)
    errs = validate_spec(
        spec, max_steps=int(getattr(get_settings(), "workbench_max_steps", 30)))
    hard = [e for e in errs if not e.startswith("warn:")]
    if hard:
        raise AuthError("레시피를 저장할 수 없습니다:\n- " + "\n- ".join(hard), status_code=422)
    return spec, [e[5:] for e in errs if e.startswith("warn:")]


@router.get("/recipes")
def list_recipes(request: Request, principal: Principal = Depends(_me)) -> dict:
    return {"recipes": _store(request).list_recipes(owner_sub=principal.subject)}


@router.post("/recipes", status_code=201, dependencies=[Depends(require_csrf)])
def create_recipe(request: Request, body: RecipeIn,
                  principal: Principal = Depends(_me)) -> dict:
    _, warns = _validated(request, body.spec)
    got = _store(request).create_recipe(
...
def get_recipe(request: Request, recipe_id: str,
               principal: Principal = Depends(_me)) -> dict:
    v = _store(request).latest_version_of(recipe_id)
    if v is None:
        raise AuthError("레시피를 찾을 수 없습니다", status_code=404)
    return v


@router.post("/recipes/{recipe_id}/versions", status_code=201,
             dependencies=[Depends(require_csrf)])
def add_version(request: Request, recipe_id: str, body: RecipeIn,
                principal: Principal = Depends(_me)) -> dict:
    _, warns = _validated(request, body.spec)
    try:
        got = _store(request).add_version(
            recipe_id=recipe_id, author_sub=principal.subject, spec=body.spec,
            derived_from_run=body.derived_from_run)
    except KeyError:
        raise AuthError("레시피를 찾을 수 없습니다", status_code=404) from None
    return {**got, "warnings": warns}
...
async def add_step(request: Request, run_id: str, body: StepIn,
                   principal: Principal = Depends(_me)) -> dict:
    """빈 런에 단계 하나. 이미 도는 단계가 있으면 **409** — 같은 단계 재실행 방지다."""
    ...
    if run["state"] in ("cancelled", "gated"):
        raise AuthError(f"이 런은 {run['state']} 상태입니다", status_code=409)

    step = Step(**body.model_dump())
    one = RecipeSpec(title="ad-hoc", steps=[step])
    hard = [e for e in validate_spec(one) if not e.startswith("warn:")]
    # 미지 변수는 빈 런에서 정상이다(앞 단계가 런 입력에 값을 합쳐 둔다)
...
async def _step_and_log(request: Request, run_id: str, step: Step, principal) -> None:
    try:
        ...
        logger.warning("워크벤치 단계 시작 실패 run=%s: %s", run_id, exc)


@router.get("/runs")
def list_runs(request: Request, principal: Principal = Depends(_me)) -> dict:
    """확인 대기가 맨 위에 온다 — 게이트에서 멈춘 런의 표면이 이 목록이다."""
...
async def ack(request: Request, run_id: str, ix: int, body: AckIn,
    ...
    version = _store(request).get_version(run["recipe_version_id"] or "")
    if version is None:
...
async def resume(request: Request, run_id: str,
    ...
    version = _store(request).get_version(run["recipe_version_id"] or "")
    if version is None:
        raise AuthError("빈 런은 재개하지 않습니다 — 단계를 다시 실행하세요", status_code=409)
    at = next((s["ix"] for s in run["steps"]
               if s["state"] in ("failed", "unknown", "pending")), len(run["steps"]))
    if at >= len(RecipeSpec.model_validate(version["spec"]).steps):
...
def cancel(request: Request, run_id: str, principal: Principal = Depends(_me)) -> dict:
    ...
    취소가 없으면 폭주하는 런을 세우는 길이 포털 재기동뿐이고, 그 순간 챗 SSE 가 전부
...
@router.post("/runs/{run_id}/save-as-recipe", status_code=201,
             dependencies=[Depends(require_csrf)])
def save_as_recipe(request: Request, run_id: str, body: SaveAsIn,
                   principal: Principal = Depends(_me)) -> dict:
    """런에서 레시피를 뽑는다 — **"하고 나서 저장" 이 성립하는 자리**다.

    어느 인자가 변수인지는 화면에서 사람이 표시하고, 여기는 그 결과를 받는다.
    """
    run = _owned(request, principal, run_id)
    steps = body.steps
    if steps is None:  # 표시 없이 저장하면 런의 단계를 그대로 굳힌다
...
async def validate(request: Request, body: RecipeIn,
                   principal: Principal = Depends(_me)) -> dict:
    """거절 사유와 경고를 저장 **전에** 보여 준다. 스키마 대조는 게이트웨이가 붙었을 때만."""
    spec = RecipeSpec.model_validate(body.spec)
    errs = validate_spec(
        spec, max_steps=int(getattr(get_settings(), "workbench_max_steps", 30)))
