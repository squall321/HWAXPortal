# 워크벤치 라우트 — 권한 **전수** 403 · 소유자 · 409 · 202 · SPA 폴백보다 위
#
...
PREFIX = "/workbench-api"


def _routes():
    """등록된 워크벤치 라우트 전수 — (method, path)."""
    out = []
    for r in app.routes:
        path = getattr(r, "path", "")
...
def _fill(path: str) -> str:
    return (path.replace("{run_id}", "r1").replace("{recipe_id}", "c1")
            .replace("{ix}", "0"))
...
def client(tmp_path):
    s = Settings(user_store_path=str(tmp_path / "users.sqlite"),
                 workbench_store_path=str(tmp_path / "wb.sqlite"),
    ...
    with TestClient(app) as c:
        ...
        app.state.workbench_store = WorkbenchStore(s)
        yield c
...
def test_라우트가_실제로_붙어_있다():
    ...
    assert f"{PREFIX}/runs" in got and f"{PREFIX}/recipes" in got
...
def test_권한_없는_계정은_모든_라우트에서_403(client):
    ...
    assert "feat:workbench" not in client.get("/auth/me").json()["entitlements"]
...
@pytest.fixture()
def user(client):
    _users(client, grants=["feat:workbench"])
    return client, _login(client, "user@corp.com")


def test_권한이_있으면_목록이_열린다(user):
    c, h = user
    assert "feat:workbench" in c.get("/auth/me").json()["entitlements"]
    assert c.get(f"{PREFIX}/runs").json() == {"runs": []}
    assert c.get(f"{PREFIX}/recipes").json() == {"recipes": []}


def test_빈_런은_게이트웨이_없이_만들어진다(user):
    """워크벤치의 진입점 — 레시피가 하나도 없어도 시작한다."""
    c, h = user
...
def test_남의_런은_404(user, client):
    c, h = user
    rid = c.post(f"{PREFIX}/runs", json={}, headers=h).json()["run_id"]
    boss = _login(c, "boss@corp.com")  # portal-admin 이지만 런은 공유하지 않는다
...
def test_저장_시점_거절이_라우트에서도_먹는다(user):
    ...
    r = c.post(f"{PREFIX}/recipes", json={"title": "t", "spec": bad}, headers=h)
    assert r.status_code == 422
    assert "gate: human" in r.text

    good = dict(bad)
    good["steps"] = [{**bad["steps"][0], "gate": "human"}]
    r2 = c.post(f"{PREFIX}/recipes", json={"title": "t", "spec": good}, headers=h)
...
def test_dry_run_미지원_도구는_저장에서_막힌다(user):
    ...
    r = c.post(f"{PREFIX}/recipes", json={"title": "t", "spec": spec}, headers=h)
...
def test_판본은_올라가고_옛_판본은_그대로다(user):
    ...
    rid = c.post(f"{PREFIX}/recipes", json={"title": "t", "spec": spec},
                 headers=h).json()["id"]
    spec2 = {"title": "t", "steps": [{"backend": "b", "tool": "a_tool"},
                                     {"backend": "b", "tool": "c_tool"}]}
    r = c.post(f"{PREFIX}/recipes/{rid}/versions", json={"title": "t", "spec": spec2},
               headers=h)
    assert r.status_code == 201 and r.json()["version_no"] == 2
    assert len(c.get(f"{PREFIX}/recipes/{rid}").json()["spec"]["steps"]) == 2
...
def test_도는_단계가_있으면_409(user):
    ...
    c, h = user
    rid = c.post(f"{PREFIX}/runs", json={}, headers=h).json()["run_id"]
    app.state.workbench_store.begin_step(rid, 0, backend="b", tool="t", args={})
...
def test_취소는_소유자와_관리자만(user):
    ...
    assert app.state.workbench_store.get_run(rid)["state"] == "cancelled"


def test_빈_런은_재개하지_않는다(user):
    c, h = user
    rid = c.post(f"{PREFIX}/runs", json={}, headers=h).json()["run_id"]
    r = c.post(f"{PREFIX}/runs/{rid}/resume", headers=h)
    assert r.status_code == 409 and "빈 런" in r.text
...
def test_stats_는_관리자만(user, client):
    ...
    assert r.status_code == 200 and r.json()["recipes"] == 0


def test_런에서_레시피를_뽑는다(user):
    ...
    r = c.post(f"{PREFIX}/runs/{rid}/save-as-recipe", json={"title": "내 절차"}, headers=h)
    assert r.status_code == 201
    v = c.get(f"{PREFIX}/recipes/{r.json()['id']}").json()
...
def test_성공한_단계가_없으면_저장할_것이_없다(user):
    c, h = user
    rid = c.post(f"{PREFIX}/runs", json={}, headers=h).json()["run_id"]
    r = c.post(f"{PREFIX}/runs/{rid}/save-as-recipe", json={"title": "t"}, headers=h)
    assert r.status_code == 422


# ── SPA 폴백보다 위 ──────────────────────────────────────────────────────
def test_API_접두사가_SPA_경로와_겹치지_않는다():
    """`/workbench-api` 와 SPA `/workbench/*` 를 가른 이유 — 겹치면 새로고침이 JSON 을 받는다."""
    assert PREFIX == "/workbench-api"
    assert not any(getattr(r, "path", "").startswith("/workbench/") for r in app.routes)
