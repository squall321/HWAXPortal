# 전문가 심층 보기 프록시 — 지식카드 목록·카드 한 장이 로그인 주체의 그룹으로 넘어가고, 실패가 빈 목록으로 뭉개지지 않는지
import json
...
def test_지식카드_목록과_카드_한_장을_그룹과_함께_포워딩한다():
    ...
    def handler(req: httpx.Request) -> httpx.Response:
        ...
        if req.url.path.endswith("/records"):
            return httpx.Response(200, json={"total": 2729, "offset": 50, "items": [{"id": "DOC-1"}]})
        return httpx.Response(200, json={"id": "DOC-1", "sections": [{"title": "본문", "text": "t"}]})

    app, c, dep = _client_with(handler, ["analyst"])
    try:
        with c:
            real = app.state.agent_client
            app.state.agent_client = httpx.AsyncClient(transport=httpx.MockTransport(handler))
            try:
                r1 = c.post("/agent/catalog/agent/records",
        ...
        assert seen[0] == ("/catalog/agent/records", {"key": "material-twin-analyst", "q": "SiC", "offset": 50,
                                                      "limit": 50, "groups": ["analyst"]})
        assert seen[1] == ("/catalog/record", {"id": "DOC-1", "groups": ["analyst"]})
