...
## D-9. 시작 화면도 같은 조직도로 — 다만 거기서는 '고르기만' 한다
챗 시작 화면(StartPicker)의 전문가 칸은 검색 목록 + **분야 드롭다운**이었는데, 그 드롭다운은
키 접두어(cam·mech·mem…)를 그대로 보여 무엇인지 알 수 없었다. 조직도는 이미 챗·심의 좌석이
같이 쓰고 있으므로 여기도 같은 화면을 연다(라벨 정본이 한 곳뿐이라 새로 만들 이유가 없다).
- 다만 **확정 시점이 다르다**. 챗 조직도는 고르는 즉시 지정하고 닫지만, 시작 화면은 도구·앱과
  함께 '적용하고 대화 시작'에서 한 번에 확정돼야 한다. 그래서 `PersonaBrowser` 에 `onPick` 을
  둬 **고른 사람을 돌려주는 모드**를 만들었다(없으면 예전처럼 곧장 지정). 버튼 문구도 갈린다 —
  '이 전문가로 고르기' / '이 전문가로 대화하기'. 시작 화면에서 ×로 닫으면 지정은 남지 않는다.
- 샘플 질의를 누르면 그 자리에서 시작한다(담아 두면 입력창만 채워진 채 멈춘다). 이때는 방금 고른
  키를 확정 함수에 직접 넘긴다 — `setAgentSel` 은 그 렌더에 반영되지 않는다.
- 뒤이어 목록 둘을 걷어냈다(사용자 지적 — 시작 화면이 길어 가시성이 나쁘다). 전문가 추천 10명과
  도구 추천 8개는 둘 다 더미 질의('전체 카탈로그 조회') 결과였고, 전문가 이름은 한 줄을 넘긴다.
  남긴 것은 **조직도 버튼 + 앱 카드**이고 개별 도구는 영역·앱·검색으로 찾을 때만 나온다.
  상단 주제 검색줄도 같이 뺐다 — 그 두 목록이 유일한 소비처였고, 전문가는 조직도 검색이,
  도구는 도구 검색이 따로 있다. 카드 높이 1,255px → 604px.
