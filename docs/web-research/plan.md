...
## 3-K. 설계 변경 — 소스별 토글 (2026-08-08 확정)

사용자 결정으로 §5-② 의 선택 구조를 바꿨다. "일반 웹을 쓸 것인가"를 조직이 한 번 정하는
전역 스위치로만 두지 않고, **소스별로 챗·심의 화면에서 켜고 끈다.**

### 왜 소스별인가

세 소스는 나가는 곳도, 필요한 승인도, 위험도도 다르다. 한 스위치로 묶으면 일반 웹 승인이
안 난 동안 공공 학술까지 통째로 못 쓴다 — 승인 리드타임이 이 로드맵의 가장 큰 도박인데,
묶어 두면 그 도박에 학술 검색까지 걸리는 셈이다.

| 토글 | 나가는 곳 | 승인 | 기본값 |
|---|---|---|---|
| 사내 자산(0층) | 나가지 않음 | 불필요 | 항상 켜짐(토글 없음) |
| **공공 학술(1층)** | arxiv·openalex·crossref·pubmed·wikipedia | 불필요(무인증·약관 명시허용) | 꺼짐 |
| **일반 웹(2층)** | api.search.brave.com | **필요** | 꺼짐 + 전역 잠금 가능 |

### 차단은 프롬프트가 아니라 바인딩에서 한다

토글이 꺼졌을 때 "쓰지 마라"고 지시하지 않는다. **도구를 아예 바인딩하지 않는다.**
모델에게 금지를 지키게 하는 것과 존재하지 않게 하는 것은 다르다.

| 경로 | 거르는 지점 |
|---|---|
| 챗 | `app.py` `_agent_for()` → `_select_tools()` 결과에서 제외. 모델의 도구 목록에 없다. |
| 심의 | `deliberation.py` 자유 조회 `_g` 딕셔너리에서 제외. 오늘 넣은 앱 좁힘과 같은 자리다. |

`pinned_apps` 로 대신하지 않는다 — 그건 "우선 사용하라"는 우선순위이고 금지가 아니다
(현재 주석에도 "다른 도구 사용 금지는 아님"이라고 적혀 있다). 사내 질의가 외부로 나가는
기능에서 그 차이는 사고와 안전의 차이다.

### 그래서 도구를 소스별로 쪼갠다 — 되돌리기 어려운 결정

`search_scholar`(1층)와 `search_web`(2층)을 **처음부터 별개 도구로** 낸다. 하나의 도구에
`source` 인자를 두는 설계는 쓰지 않는다. 인자로 받으면 바인딩 단계에서 소스를 가릴 수 없어
토글이 프롬프트 지시로 퇴화한다. 이건 P2 에서 정해지면 뒤집기 어렵다.

### 3층 옵션 구조

| 층 | 어디 | 성격 | 누가 정하는가 |
|---|---|---|---|
| 전역 | `SEARCH_MODE=offline\|domain_only\|full` (env) | 하드 차단. 소켓을 안 연다 | 조직 |
| 대화 | 챗 컴포저 위 · 심의 `DelibOptsPanel` 체크박스 | 바인딩 포함/제외 | 사용자 |
| 요청 | 나가기 직전 `queries_sent` 고지 | 확인 | 사용자 |

**전역이 끄면 대화 토글은 비활성으로 보인다.** 조직이 "일반 웹 금지"를 정했는데 사용자가
UI 에서 켤 수 있으면 안 된다. 이것이 1층과 2층을 분리해 두는 실질적 이유다.

### UI 자리 — 새로 만들 것이 거의 없다

심의는 `frontend/src/components/chat/DelibOptsPanel.tsx` 가 이미 `<details>` 체크박스
패널이다. 손잡이 6개가 있는 그 리스트에 두 항목을 더한다. 챗은 컴포저 위에 같은 모양의
패널을 하나 만들고 같은 CSS 를 쓴다. 계약은 `delib_opts.sources: ["scholar","web"]` 와
챗의 `search_sources: ["scholar","web"]` 다.

### 단계 반영

- **P2** — 도구를 소스별로 쪼개 등록한다(`search_internal` 만 우선, 계약 형태 확정).
- **P4** — `search_scholar` 추가. 여기서 대화 토글 UI 를 붙인다(켤 대상이 처음 생기는 시점).
- **P7** — `search_web` 추가. 전역 잠금과 토글의 상호작용을 여기서 검증한다.

토글 UI 를 P2 에 먼저 만들지 않는다 — 켤 대상이 없는 체크박스는 켜도 아무 일이 없어,
사용자에게 '고장난 기능'으로 보인다.
