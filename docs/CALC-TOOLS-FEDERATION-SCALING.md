...
## 3b. 운영 이슈 (2026-07-31 실측 — 계산 도구 배포 중 반복 발생) — **3건 처리 완료**

| 이슈 | 처리 | 커밋 |
|---|---|---|
| 게이트웨이 프로세스 수명 | `start.sh --bg`(setsid 세션 분리 + pidfile/로그 + 헬스 대기) + **포트 선점 가드**(정상 가동 중이면 중복 기동 생략, 남의 프로세스면 PID/커맨드 표시 후 실패) + `GATEWAY_PORT` 오버라이드로 "임시 실행은 다른 포트" 규약을 코드화. 기본 포그라운드 동작은 불변이라 오케스트레이터·systemd 경로 영향 없음 | gateway `c0ce35f` |
| G3 도구 재집계 | revive 루프가 **연결 상태 백엔드도 `list_tools()`로 확인** — 실패하면 죽은 세션으로 보고 재연결, 도구 구성이 바뀌었으면 재집계. `_call_tool` 의 재연결은 `_REAGG` 플래그로 재집계를 예약(호출 지연 없음) | gateway `c0ce35f` |
| 카탈로그 버전 라벨 | 스캐너가 매 스캔마다 manifest version 과 앱 헬스의 version 을 대조 — 불일치 시 경고 + `App.extra['version_drift']` 기록(해소되면 자동 제거). 헬스 경로는 `launch.health_check.path` 를 따름. 버전 미노출 앱은 생략 | HEAXHub `7cf1420` |

**왜 systemd user unit 승격이 아니라 위 방식인가.** `services.py` 는 이미 `start_new_session=True`
로 분리 기동하므로 오케스트레이터 경유 프로세스는 세션 종료에 죽지 않는다. 실제 사고는 전부
**손으로 `./start.sh` 를 돌린 경우**였고, 그건 `--bg` 로 해결된다. 게다가 `hwax-stack.service`
주석이 밝히듯 이 스택은 "서비스별 유닛이 아니라 오케스트레이터를 부르는 단일 유닛" 설계라,
tier 16 만 별도 유닛으로 떼면 관리 주체가 둘로 갈린다. 크래시 자동 재기동이 필요해지면 그때
단일 유닛에 supervision 을 얹는 편이 설계와 어긋나지 않는다.

### 원본 이슈 기록

| 이슈 | 실측 내용 | 제안 |
|---|---|---|
| **게이트웨이 프로세스 수명** | `./start.sh`가 `exec`으로 포그라운드 실행이라 호출한 셸/세션이 끝나면 함께 죽는다. 이번 세션에서만 3회 재기동 필요했고, 한 번은 다른 세션의 임시 repro 프로세스(포트 오버라이드·/tmp config)가 9110을 점유한 채 돌고 있었다 | `services.yaml`의 tier 16 기동을 systemd user unit(또는 오케스트레이터가 `setsid`로 분리)으로 승격. 임시 실행은 반드시 다른 포트를 쓰도록 규약화 |
| **G3 도구 재집계** | heax 백엔드 앱의 **인스턴스가 교체되면**(SIF 재빌드·재기동) 게이트웨이가 세션은 재연결하지만 **도구 목록은 재집계하지 않는다** — 새 도구가 게이트웨이 재기동 전까지 안 보인다 | `_revive_loop`에서 재연결 시 `list_tools()`를 다시 받아 카탈로그를 갱신 |
| 카탈로그 버전 라벨 | manifest version과 앱 실제 버전이 어긋나면 카탈로그가 구버전을 표시(스캐너는 manifest 기준) | 앱이 `/health`에 버전을 노출하면 스캐너가 대조·경고 |
