...
- [x] 게이트웨이 정확이름 차단이 **별칭으로 우회**되던 것 — 닫고 **절차만 면제**(W-100, 사용자 결정).
      열쇠는 사람이 못 만드는 `purpose: "procedure"` 클레임이다(`pat_name` 은 사용자가 정하므로 못 쓴다).
      면제는 정확이름 목록에만 — `delete_`·`cancel_` 은 절차도 못 부른다. **배포 필요**(양쪽 재기동)
- [x] 면제의 전제("포털이 사람 확인을 받았다")를 참으로 — MUST_GATE 는 저장 판본에 `gate` 가 없어도 실행 시점에
      멈추고(`_gate_of`), 즉석 단계는 사람 자리를 422 로 거절한다(`StepIn.gate` 가 말없이 버려지던 것 포함).
      면제 호출은 감사원장에 `purpose` 칸으로 남는다
- [ ] 운영 감사원장의 시험·재현 흔적 약 247줄(성공 모양 47줄) — **사용자 결정 대기**(W-100)
...
- [x] **결정**(2026-09-17, W-93) — DynaForge 후처리 연산이 클러스터에 잡을 걸고 **HTML 만 받는다**. 길은 대시보드 REST 기본·ssh
...
- [x] **결정**(2026-09-17, W-93) — 자동 반입 리포트는 **② 예약자 명의 + 소속 단위 읽기 공유**(기준: 과제 간 비교).
      과제 키는 목록에서, 잡 하나에 리포트 하나(외부 잡 키 유일), 해석 조건을 메타로
      ⚠ 어휘의 자리는 AI DataHub 이 아니라 **RA 기준정보 `project`(과제)** 다 — 실측으로 정정(W-99)
...
- [x] 소속 전달 — `X-Heax-User-Affiliation` + `X-Heax-Aff-Proof`(hmac) 를 **호출마다** 싣는다(W-98).
      ⚠ 평문 한 줄이던 첫 판은 **권한 상승**이었다 — 앱의 정문이 게이트웨이 하나가 아니고(직접 :8443/mcp),
      Caddy 가 지우는 위조 헤더는 Email·Name 둘뿐이다. 서명은 `v1|email|aff|exp` 라 남의 호출엔 못 쓴다.
      포털은 표에 없는 소속 id 를 안 내보내고, 게이트웨이는 낡은 소속을 안 쓰며 응답 캐시 키에 소속을 넣는다.
      요청서 §8 개정 발송(KooRemapper c10a1d5). **배포 전** — 양쪽 재기동 필요
- [x] 포털 — 평문 결과에서 잡 ID 추출(W-94) — `save: {slurm_job_id: "re:job_id=(\\d+)"}`, 제출 씨앗 둘에 적용
- [x] cae00 절차 미동작 점검·수정(2026-09-17, W-95) — 게이트웨이 실패를 502+이유로, 드리프트 이유를 원장에,
      update-all 헬스게이트에 절차·dist 신선도, git pull 실패를 종료코드로
- [x] 포털 — 회수/부품 보고서 씨앗(W-96) · 축 태그 붙이기 R2c(W-97)
- [ ] 포털 — 제출 씨앗 **예약 단계**(DynaForge 연산이 와야 한다) + 그 앞의 **과제 목록 단계**
      → 목록 선택은 새 기계가 필요 없다(목록 도구 + `select {on_many: ask}`). 어휘가 dev 에 0건이고
      붙을 자리가 예약 단계라 **예약과 같이 막혀 있다**(W-99)
...
- [ ] `sim_type` 만 바꾼 판본 둘 — `fullangle_drop`(각도 프리셋) · `partial_impact`
      ...
      100mm 초과 경고. 테스트 11(`test_procedures_s3.py`, 되돌림 셋 확인)
      → **전각도 완료** — 씨앗 `fixtures/fullangle-drop-submit.yaml`: 같은 틀, 프리셋 enum·바닥 종류 enum·
      **바닥 물성으로 프리셋의 SI 값을 덮는다**·높이 100mm 이하 경고. 두 씨앗 공통 검사로 묶어 테스트 21(되돌림 둘 더)
...
- [x] `board_type` — 어휘 대응·HALF/FULL 판정 규칙이 없으면 '사람이 채운다' 로 분류
      → 뜻 확정(사용자 2026-09-16): INT=인터포저 보드·HALF=인터포저 없는 모바일·FULL=태블릿 등 큰 보드.
