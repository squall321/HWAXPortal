...
- [ ] 화면 — 후보 목록에서 고르기(지금은 API 만)
- [ ] 룰 변수의 예시·설명을 `find_*` 스키마에서 끌어온다(글롭 문법이 거기 적혀 있다)
- [ ] **② 환원 — 스택업** (층 순서·두께·재질·법선·외곽) → StepForge 결손 ①
- [ ] ② 두 번째 환원 도구가 `bend_profile` 과 **같은 모양인지** 보고 규약으로 올린다(§10-7)
- [ ] **③ 보강 — 라미나 물성**(강도·피로) 조달 경로 → MaterialTwin ↔ 적층 해석기 결손
- [ ] S5 일괄 재생과 잇기 — 룰이 N개를 고르면 **전부 돌리는** 갈래(지금은 하나만 고른다)
...
- [ ] `activity[]` 에 `ts`·`ms` — 지금은 소요·시각이 안 남는다(원장에는 있다)
- [ ] 합격 기준 확인 — `predict_sed` 병렬 5회가 **각각 다른 인자에 짝지어지는지** 실제로 본다
- [ ] 초안을 확정해 절차로 굳히는 화면(변수/상수 고르기)
