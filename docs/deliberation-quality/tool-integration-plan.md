...
- **1-E** 워크플로 좌석의 MCP 출력 상한 — **실측 종결(2026-09-03)**: 상한 초과 결과는
  ...
  행동 편차뿐 — 구조 결손 아님.
- **1-F** 형제 파이프라인 3본 — **감사·수리 완료(2026-09-03, 커밋 9064d2e)**. 32에이전트
  (감사 3 + 발견별 적대 반박) → 확정 19 / 기각 9. 치명 5: risk-review 절단본 원장 기계병합
  ·에코 제출 무예산(의역 변조)·TURN_CAP 선두 채움(최종 라운드 전원 유실), test-plan 근거
  null 무감지, viz 평균 0.0 날조. 전부 수리 + 단위검증(126건→최종 21/21 보존 등).
  test-plan 의 자식 플래그 승계는 온전함이 확인됐다(결함 아님 판정 1건).
...
- **(3) 인용 강제 — MCP 경로 구현 완료(2026-09-03)**. EV [e:N] 안정 id + 좌석·의장
  인용 지시 + 결정문 수치의 결정적 후검증(citationAudit — 대조 원문은 근거 **전문**·
  정량 근거·전 라운드 발언). e2e 카나리아 심의로 정카나리아 대조·역카나리아 검출
  (의장 신규 산술 3.22 정확 검출) 확인. **잔여**: 웹 엔진(deliberation.py) 파리티
  이식, (경험칙) 무표기 수치 검증은 이 채널의 unmatched 목록이 사실상 커버.
  부수 관찰: 좌석이 사내 ADC12 조회에 material_lookup(DynaForge 사용 집합)을 골라
  '기준선 0건'으로 오판 — MaterialTwin get_material 에는 있다. 도구 입구 안내
  (material_lookup ↔ get_material 구분)가 다음 설명 보강 후보.
