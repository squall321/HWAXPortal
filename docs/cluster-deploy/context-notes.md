...
### 구현 착수 보류 — 계획 경화 먼저 (2026-08-02, 사용자 결정)

- "지금 당장 시작하기에 리스크가 있으니 계획을 단단하게" — Phase 0 포함 전 구현 보류.
  착수 게이트(checklist 상단)와 §14 검증 실험 목록을 신설.
- 경화 1회전에서 잡은 것: **ln -sfn 이 원자적이라는 자체 오류**(→ tmp+mv -T 프로토콜로 정정),
  스테이징 tmp 가 /data 밖이면 원자성 붕괴, venv 공유 시 __pycache__ 경합, 전역 배포 뮤텍스
  부재, 부분 실패 시 혼합 버전 상태 규칙 부재(→ 1버전 하위호환 가정 명시), singleton 락에서
  pid 가 노드 간 무의미한 문제(→ heartbeat + 명시적 takeover 만).
