...
- **cae00(현 운영)**: 클러스터가 검증 완료될 때까지 **현행 방식 그대로, 손대지 않는다.**
  전환은 마지막에 별도 결정.
- **RA**: 소스 무수정 원칙 유지. 단 **RA DB 를 백업 대상에 편입**(포털 레포 변경만으로 가능 —
  현행 backup-local.sh 는 "RA hands-off" 로 DB 까지 제외 중인데, 실데이터가 있는 유일한
  서비스가 RA 이므로 이 제외는 위험하다).
...
5. **백업 없이 이관 없음.** RA DB 백업 편입 + 복구 리허설 성공이 Phase 2 진입 조건.
...
- **0.2 RA DB 백업 편입 + 복구 리허설**: backup-local.sh 에 RA pg_dump 추가(포털 레포만 변경).
  임시 DB 에 실제 복원해 로그인·보고서 열람까지 확인. **전체 계획의 첫 실행 항목.**
...
- **3.2** RA 이관: placement 등록(기동은 자체 start.sh — 소스 무수정), RA PGDATA /data/pg 이동
  (사전 스냅샷 필수), 포털 라우트 endpoints 화.
