...
이웃 함정 — **Gemini CLI 는 신뢰하지 않는 폴더에서 user 범위 MCP 서버를 끄거나 숨긴다.**
격리 패치로 배치가 안 죽어도 Gemini 등록만 실패/`Disabled` 로 보일 수 있는데, 이건 우리
버그가 아니라 Gemini 의 폴더 신뢰 정책이다(다운로드 폴더에서 바로 실행하면 특히). 배치가
그 사유를 화면에 짚어 주도록 해 뒀다 — 안 그러면 사용자가 원인을 배치에서 찾는다.
