...
> ① 의 후반(챗 기록 → 재생)은 조사 결과 선택 단계 S5 로 내렸다 — 이유는
...
- `notes`(JSON, 4KB) — 도구가 결과 속에 넣어 준 경고·모델 출처·버전을 어댑터가 뽑아 올리는 칸.
  §5-4 의 "경고를 반드시 싣는다" 를 받는 자리이고, S4 비교표가 열로 보인다.
...
StepForge·DynaForge·RA 를 HWAXRisk `project_id` 아래 **부분적으로** 묶고 있다 — S2 는 0 에서
...
- **must-gate 목록** — `publish_report`·`request_unpublish`·`trash_report`·`restore_version`·
  ...
  `ingest_*`·`train_model` 은 must-gate 가 아니라 저장 시 **경고 등급**으로 두어 S4 일괄 재생을
  살린다.
...
있지만 **핵심 기능의 선행 조건은 아니다** — S5 의 전제일 뿐이다.
...
'미검증' 으로 표시한다. 재생(S4 포함)은 이 모드로 시작해 사람이 끄고, 단계별 실행은 끈 채
시작한다.
...
| **S2** | 과제 키 레지스트리 | 중간 | S3(또는 S4 안) |
| **S4** | 일괄 재생(과제 N건 + 비교표) | 작다 | S1·S3 |
| **S5** | 챗 → 레시피 제안 *(선택)* | 불확실 | 관측 개선 |
| **S6** | 리스크 패턴화 | 중간 | S4 |

**S0 과 S1 은 병렬이다.** S1 의 첫 레시피는 ODB 없이 dev 에서 완주되는 것으로 잡는다 — 예를
들어 파싱된 과제 `project_id` 를 변수로 → `list_parts` → `create_report_draft`(gate). 잡을 거는
도구(`intake`)는 레시피 밖이다(§4). **S2 는 뒤로 뺐다** — 작지 않고(앱마다 시야가 다르다)
v1 목표 레시피의 임계 경로에 있지도 않다(context-notes W-22).

**S4** — 배치는 첫 `gate: human` 직전까지 돌고 비교표를 만든다. 게이트 이후 단계는 표에서
과제를 골라 개별 재개한다(초안 N개 자동 생성 금지). 동시 상한은 "같은 백엔드에 동시 N" 이다.

**S6 첫 항목은 런 → 심의 다리다.** 런 상세에 "심의로 넘기기" 를 두고 단계 기록을
...
승격을 검토한다. 못 미치면 접거나 S2~S4 를 보류한다. 세 수치는 근거 있는 값이 아니라
...
| 3 | S1 의 첫 레시피를 무엇으로 | ODB 없이 dev 에서 완주되는 것 권장(§6 예) |
| 4 | 일괄 재생 상한 | S4 때, 같은 백엔드 기준 |
...
| 8 | S2 를 HWAXRisk `rr_sources` 확장으로 할지 신규로 할지 | §5-3. S3 뒤에 |
