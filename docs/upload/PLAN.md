...
- 청크 재개(resumable)·진행률 바 같은 대용량 UX 고도화. (단순 디스크 스트리밍은 1차 필수)
...
- **앱 레벨 하드 상한은 두지 않는다.** 처음엔 상한을 두려 했으나 근거가 틀렸다 —
  422 페이로드 폭발은 JSON 본문 얘기지 multipart 파일에는 안 맞는다. 파일에서 진짜
  위험한 건 크기가 아니라 **메모리**다. 그래서 (a) 파일을 디스크로 스트리밍해
  통째로 메모리에 안 올리고, (b) nginx 가 이미 거는 2GB(client_max_body_size 2048m,
  hwax.conf:25)를 바깥 경계로 쓴다. 임의 하드 상한은 정당한 큰 파일을 막을 뿐이다.
