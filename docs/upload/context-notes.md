...
## D-11. `intake` 로 되돌렸다 — D-8 을 뒤집는 게 아니라 그 교훈을 적용한 것

00:37 에 StepForge SIF 가 재배포되면서(게이트웨이 250 → 266) `intake`·`set_project_meta`·
`get_project_meta` 가 살아났다. 그래서 D-8 에서 편 묶음을 다시 `intake` 한 번으로 접었다.

되돌린 결정적 이유는 "이제 있으니까"가 아니다. **메타를 담는 `set_project_meta` 가
`intake` 와 같은 커밋(c8fab07)이다.** 즉 메타 폼 자체가 그 배포에 의존하므로, `intake` 를
피해도 배포 의존성이 하나도 줄지 않는다. 그렇다면 4번 부를 이유가 없다.

> D-8 의 교훈은 "`intake` 를 쓰지 마라"가 아니라 **"소스 말고 게이트웨이 `tools/list` 로
> 확인하라"** 였다. 확인했고, 있다.

## D-12. 과제 메타는 **새 과제일 때만** 보낸다

기존 과제에 파일 하나 붙였다고 그 과제의 과제번호·부서·목적·특이사항을 덮는 건 남의
기록을 지우는 일이다. 그래서 폼은 `mode === 'new'` 에서만 뜨고, 백엔드도 기존 과제
분기에서는 메타 도구를 아예 부르지 않는다. 클라이언트가 메타를 실어 보내도 무시된다.

가드로 확인했다 — 기존 과제에 `code:"HACK-999"` 를 실어 두 번째 파일을 붙였을 때
메타는 그대로였고(`ORIG-001`) 파일만 1 → 2 로 늘었다.

## D-13. 개발단계는 정규 컬럼이 아니라 `extras.stage` — 대신 값을 묶는다

StepForge 의 정규 칸은 `code·owner·department·purpose·note` 다. 개발단계(선행·DV·PV·양산)는
없어서 자유 항목 `extras` 로 간다. 자유 항목이지만 **값은 고정 목록으로 묶었다** — 자유
문자열이면 나중에 "DV 과제만" 같은 집계가 안 되기 때문이다. 목록 밖 값은 버리고,
안 고르면 키 자체를 넣지 않는다(빈 문자열을 남기면 '값이 있는데 빈 것'과 구분이 안 된다).

프론트 `STEP_STAGES` 와 백엔드 `_STEP_STAGES` 가 같은 목록이어야 한다 — 어긋나면 사용자가
고른 값이 조용히 버려진다.
