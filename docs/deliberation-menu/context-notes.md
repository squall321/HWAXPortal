...
## 2026-07-18 프로덕션 첫 실행 이슈와 수정

**증상(cae00).** ① SF `daily_briefing`이 `relation "voc_active" does not exist` — SQL 에러 원문이
환기 블록에 그대로 표시. ② 이후 재시도는 계속 "(응답이 없습니다)".

**원인 3겹.**
1. cae00 SF postgres에 `voc_active` 미존재 — **SF DB가 Drive에서 미복원**(dev는 180,000행 정상).
   코드 문제가 아니라 데이터 배포 문제.
2. 환기 단계가 도구 반환을 내용/에러 구분 없이 수용 → 에러 원문 유입.
3. 심의 경로 error 이벤트가 `{content}` — 프론트 계약은 `{code,message}` → 에러가 화면에
   빈 응답("(응답이 없습니다)")으로만 보임. 또 제너레이터가 done 없이 죽으면 프론트 streaming
   잠금이 영구화되어 재전송이 무시됨.

**수정.** `_tool_text_ok` 필터(+미가용 시 "조회 불가, 질문 기반 진행" 한 줄), error 계약 정합,
`run_deliberation` 최상위 가드(항상 error+done), `chat.api.ts` 스트림 종료 시 onDone 1회 보장.
검증: 미가용 스텁 단위테스트(에러 원문 유출 0) + 정상 SF 회귀(부정급증 1건 → "심의에 포함" 분기 라이브 확인).
커밋: agent-server fe65d65, portal 89c2def.

**cae00 조치 필요.** SignalForge DB 복원(`SignalForge/scripts/sync-from-drive.sh` 또는
`deploy-all-from-drive.sh signalforge`) 후 SF MCP 재기동. AIDataHub DB 복원 여부도 함께 확인
(recommend_agents가 비면 심의 자체가 "no_personas"로 중단됨 — 이제 그 이유가 화면에 뜬다).
