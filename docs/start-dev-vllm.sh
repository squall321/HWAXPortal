...
# dev 한정 참고 — 이 GPU 는 AIDataHub API 와 공유한다. 실측(2026-09-10, 15.92 GiB) —
#   vLLM 0.80 = 12.85 GiB · AIDataHub 정상 1.11 GiB (여유 1.38).
#   단 AIDataHub 는 **기동 중 피크가 ~3.1 GiB** 라, 그 순간에 vLLM 을 띄우면 VRAM 부족으로
#   EngineCore 가 죽는다. 잠시 뒤 다시 띄우면 된다. 급하면 .env 에 VLLM_GPU_MEM_UTIL=0.70.
#   ⚠ 운영 타깃은 이 로컬 vLLM 이 아니라 상암 B300 GLM 이다(HWAXAgentServer/.env). dev 편의용.
