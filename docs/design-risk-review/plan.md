...
이 문서는 HWAX Portal 에 '리스크 심사' 메뉴를 붙이는 구현 계획이다. 과제의 MCAD(StepForge)·Dyna(DynaForge)·ECAD(ODB hub 어댑터 계약, 스텁) 소스를 읽기 전용으로 읽어 불변 설계 IR 스냅샷(rr_ir)으로 동결하고, 단일 스냅샷의 상태(rr_state)와 두 스냅샷의 3층 diff(rr_diff)를 결론 없이 코드가 정리하며, HW/XD 전문가 로스터 ≈350석이 한 번씩 자기 도구를 실호출해 낸 finding·gain·성격 서술을 IR id 에 앵커된 원자로 포털 sqlite·ReportArchive KG(관리 API·MCP 만, RA 코드 무수정)·AIDataHub 3층에 저장하고, 그 저장물을 다음 과제와 계보 없는 다른 connectivity 의 심사 브리프에 원천 그대로 되먹여 '리스크 심사 보고서' 를 낸다. §0 이 전 절이 공유하는 이름의 정본이고, §1~§10 은 그 이름만 써서 스키마·임계·상태 전이·API 인자·통과 기준을 적는다. 기존 심의 메뉴·엔진·소스 앱은 additive 변경(dict 항목·조건부 1줄·플래그 조건부 라우터)만으로 잇고 기존 기능은 손상하지 않는다.
...
| §5 | 저장과 재사용 — sqlite DDL·RA KG 6축 12관계·AIDataHub doc_type·id 연결·prior_evidence E0~E9·유사 검색·다른 connectivity 회수 |
...
| §8 | 포털·MCP 통합 — 프론트·REST·인증·MCP 파리티·hwax-risk·무손상 변경 목록·반영 절차 |
| §9 | 단계 계획 — P0~P7 목표·산출물·의존·수치 통과 기준·리스크 |
| §10 | 열린 질문·결정 필요 — 승인·보안·소스 계약·저장소·전문가 범위·어휘·예산·워크플로 |
...
| 용어 | 정의 |
|---|---|
| 과제(project) | 심사 단위인 설계 프로젝트. 포털 `rr_projects` 행이 권위 id 이고 RA `project` 축에 code=포털 id 로 복제된다. (§2) |
...
| parse_risk_spec | parseSimSpec 복제 파서(펜스 우선 → 마지막 균형 중괄호 → 실패 null 비치명). 포털 단일 구현(`narrative.py`)이고 P0 산출물이다. MCP 경로 결과도 `/panels/{id}/complete` 로 보내 같은 파서를 탄다. (§4) |
...
| runner | `runner.py` 백그라운드 실행기. 세마포어 `risk_concurrency`, 포털 `/api/agent/chat` 루프백, SSE 캡처, 파싱·병합·외부 반영·완결 판정. (§6) |
...
| quality.flag | 패널 품질 경고 값 `over_budget · rescreen_seats · coverage_mismatch · header_mismatch · low_tool_use · low_ir_cite · adversary_silent · adversary_overreject · spec_parse_failed`. 실패가 아니라 표기다. (§6) |
...
| hwax-risk | 포털 MCP 앱(`mcp_server.py`, 게이트웨이 `gateway_config.json` 백엔드 항목). 도구 `risk_get_snapshot · risk_get_diff · risk_get_registry · risk_claims_for_ref · risk_submit_panel_result`. (§8) |
| L1 / L2 파리티 | L1 = `hwax-deliberate chairTemplate:'risk-review'` 단발(원장 미연동). L2 = `hwax-risk-review.js` 오케스트레이터가 포털 `/brief` 를 받아 패널을 돌리고 `/panels/{id}/complete` 로 되돌림. (§8) |
| check_chair_parity.py | PY/JS 의 `CHAIR_ITEMS · CHAIR_ADVERSARY · RISK_SEAT_CONTRACT · 제목 삼항` 문자열 바이트 동일 검증 스크립트. (§8, §9) |
| 사용자 PAT 대리 발급 | 러너가 타깃 owner 의 `principal_json` 으로 패널마다 사용자 PAT 를 발급하는 경로. 발급 함수는 `_chat_user_pat`(routes.py:158)의 형제 `issue_user_pat` 이고 audience 는 루프백·게이트웨이 MCP 모두 `pat_chat_audience`(deps.py:93)·rest_proxy 만 `'heax'` 로 확정이다(§8.2.4). P0 (9)(a) 는 '발급 PAT 로 루프백 200' 통과 여부만 실측하고, 실패하면 '사용자 브라우저 세션에서만 패널 실행(무인 배치는 dyna absent 강등, 도구 귀속은 `/panels/{id}/complete` 의 `events[]`)' 폴백이다. (§8, §9, §10) |
...
| 이름 | 형식 |
|---|---|
| snapshot_id · diff_id · panel_id · job_id · opinion_id · finding_id(행) · character_id · narrative 계열 id | 포털 uuid hex32(conv_store `_uid` 패턴) |
...
| RA 인스턴스 | `code=<포털 id>(≤64)`, `value='<과제코드>@<source>@<YYYYMMDD-hhmm>'` |
...
### 0.3.1 포털 sqlite `data/risk_review.sqlite`(`risk_store.py`, ID 권위)
...
| rr_id_map | 포털 id ↔ RA entity id ↔ AIDataHub record id 보조 매핑(§5) |

공통 규약. 모든 표에 `owner_sub` 를 두고 조회는 owner_sub 로 좁힌다. 조직 공유 자산(rr_findings·rr_registry·rr_patterns·rr_metrics·rr_iface_alias·rr_delta_priors)은 `visibility ∈ private\|org` 컬럼을 두되 기본값은 private 이고 P6 에서 결정한다(§10).
...
### 0.3.3 AIDataHub(REST `POST /api/records/import?external_source=hwax-portal` + `_external_id` UPSERT, `create_doc_type`·`create_agent` 1회)
...
### 0.4.1 백엔드 신규 `HWAXPortal/backend/app/risk/`

| 파일 | 역할 |
|---|---|
| `__init__.py` | 패키지, `risk_review_enabled` 조건부 라우터 등록 진입점 |
| `risk_store.py` | RiskStore(sqlite3+Lock, CREATE TABLE IF NOT EXISTS, owner_sub 강제) |
| `adapters/base.py` | `IrAdapter{kind, version, discover(registry), capture(ref, principal) -> AdapterResult}` |
| `adapters/registry.py` | 게이트웨이 `/tools-map`·manifest 로 소스 앱 발견(앱명 하드코딩 금지) |
| `adapters/mcad.py` | StepForge REST 원문(`/tree`, `/artifacts/graph/`, `/parts`)+MCP 읽기 |
| `adapters/dyna.py` | DynaForge `inspect_file`·`report_*` 읽기, 브리지 검증, `_group_of` 재구현 |
| `adapters/ecad_stub.py` | ODB 계약 4도구 존재 검사, 부재 시 `ecad_absent` |
| `adapters/ecad.py` | P7 실연동(계약 이행 시) |
| `ir_builder.py` | rr_ir 조립·ir_hash·rollups·dims_named 재평가·원장 재적용 |
| `sameas.py` | same-as 사다리·헝가리안·ckey 부여·원장 재적용 |
| `diff.py` | 3층 diff·임계·comparability·rollup·의미 이벤트 |
| `state.py` | rr_state·게이트 G1~G7·signals·character_seed·feature_vector·rule_hits |
| `render.py` | 정규 표기·summary_text·판단어 린터 |
| `taxonomy.py` | taxonomy.v1.json 로더·버전·unclassified 처리 |
| `planner.py` | Tier·라운드로빈·인접 표·deferred·carried 편성 |
| `runner.py` | 배치 러너·세마포어·루프백 `/api/agent/chat`·SSE 캡처·상태기계 |
| `registry.py` | 등록부 병합·완결 판정 C0~C3·통합 보고서 조립 |
| `narrative.py` | parse_risk_spec·cites 해석·seat_opinion 추출·prior_evidence(E0~E9) |
| `character.py` | character_profile 합성·승격 |
| `ra_client.py` | RA 관리 REST·MCP 4종 래퍼(멱등 upsert, get_object 후 병합) |
| `adh_client.py` | AIDataHub REST import UPSERT·doc_type·agent |
| `routes.py` | FastAPI 라우터 prefix `/api/risk`, 인증 `principal_pat_or_session` |
| `mcp_server.py` | 포털 MCP 앱 `hwax-risk`(P5) |
| `schemas/{rr_ir, rr_state, rr_diff, risk_spec, seat_opinion}.v1.json` | JSON 스키마 + 유효/무효 픽스처 |

### 0.4.2 백엔드 기존 파일 변경(additive)

`HWAXPortal/backend/app/config.py`(Settings) · `HWAXPortal/backend/app/main.py`(lifespan·router include) · `HWAXPortal/backend/app/agent/routes.py`(DelibOpts personas `origin`, kind 화이트리스트 `risk-review`) · `HWAXAgentServer/deliberation.py`(`_CHAIR_ITEMS['risk-review']`, `_CHAIR_ADVERSARY['risk-review']`, doc_title, `_RISK_READ_TOOLS`, `_RISK_KEEP_TOOLS`, `_RISK_SEAT_CONTRACT`, `_resolve_opts` origin 통과) · `HWAXPortal/infra/pipeline/hwax-deliberate.js`(`CHAIR_ITEMS`·`CHAIR_ADVERSARY`·`RISK_SEAT_CONTRACT`·제목 삼항).

### 0.4.3 스크립트·테스트

| 파일 | 역할 |
|---|---|
| `HWAXPortal/backend/scripts/bootstrap_ra_ontology.py` | 6축·12관계 멱등 부트스트랩(dry-run 기본) |
| `HWAXPortal/backend/scripts/bootstrap_adh.py` | doc_type·의사 에이전트 생성 |
| `HWAXPortal/scripts/check_chair_parity.py` | PY/JS 문자열 바이트 동일 검증 |
| `HWAXPortal/infra/scripts/sync-workflows.sh` | `.claude/workflows` 사본 동기(기존 파일, 실경로) |
| `HWAXPortal/backend/tests/risk/` | pytest(스키마 라운드트립·게이트 픽스처·diff 합성 쌍·상태기계 불변식·편성 결정론·SSE 귀속) |
| `HWAXPortal/backend/tests/risk/golden/sif-e2e.ir.json` | 골든 IR |
| `HWAXAgentServer/tools/delib_metrics.py` | 리스크 지표 7종 추가(실경로) |

### 0.4.4 프론트(`HWAXPortal/frontend/src/`)

| 파일 | 변경 |
|---|---|
| `components/chat/delibTaxonomy.ts` | JobId·JOBS·JOB_ROUTING 에 `risk-review` |
| `api/conversations.api.ts` | ConvKind `risk-review` |
| `api/risk.api.ts`(신규) | `/api/risk/*` 클라이언트 |
| `App.tsx` | 라우트 `/risk-review`, `/risk-review/projects/:id`, `/risk-review/compare`, `/risk-review/targets/:key` |
| `components/layout/AppHeader.tsx` | NavLink '리스크 심사'(플래그 조건부 표시) |
| `pages/risk/RiskHomePage.tsx` | 과제 카드·등록 |
| `pages/risk/ProjectPage.tsx` | 소스 카드·스냅샷 동결·게이트·dims 정의·성격 프로파일·유사 과제 |
| `pages/risk/SnapshotPage.tsx` | 개요·노드/엣지 표·mermaid·dims_named·rule_hits·character_seed |
| `pages/risk/ComparePage.tsx` | base/target 선택·SameAsResolver·GateBanner·DiffView |
| `pages/risk/TargetPage.tsx` | 진행판·패널 목록·등록부·verdict·PanelRunner |
| `pages/risk/components/{SameAsResolver, GateBanner, DiffView, PanelRunner, RecallPreview, CoverageHeatmap}.tsx` | 부품 |

### 0.4.5 파이프라인·게이트웨이·문서

| 파일 | 역할 |
|---|---|
| `HWAXPortal/infra/pipeline/hwax-risk-review.js` | L2 오케스트레이터(+`.claude/workflows` 사본) |
| `HWAXMcpGateway/gateway_config.json` | `rest.heax`·`portal.audience_ok` 설정, (P5) `hwax-risk` 백엔드 항목 등록(코드 변경 0, §8.2.5·§8.3.5) |
...
| `HWAXPortal/docs/design-risk-review/taxonomy.v1.json` | 택소노미 |
| `HWAXPortal/docs/design-risk-review/character-vocab.v1.json` | 성격 통제 어휘 |
| `HWAXPortal/docs/design-risk-review/character-seed-rules.v1.json` | 씨앗 규칙표 |
| `HWAXPortal/docs/design-risk-review/adjacency.v1.json` | 도메인 인접 표 |
| `HWAXPortal/docs/design-risk-review/rules-seed.v1.json` | rr_rules 시드 6종 |
| `HWAXPortal/docs/design-risk-review/seat-contract.v1.json` | 도메인별 좌석 계약표(엔진 상수·E0c 의 원천) |
| `HWAXPortal/docs/design-risk-review/odb-adapter-contract.md` | ODB 어댑터 계약 |
| `HWAXPortal/docs/deliberation-quality/method-menu/decision-table.md` | Job 8행째·계약·파리티·기존 심의 영향 기록 |

문서 디렉터리는 `docs/design-risk-review/` 로 고정한다(1차 합성의 `docs/risk-review/` 표기는 이 이름으로 통일).

## 0.5 API·MCP·Settings 이름 총목록

### 0.5.1 REST `/api/risk`(정의는 §8)

`POST /projects` · `GET /projects` · `GET /projects/{id}` · `POST /projects/{id}/sources` · `POST /projects/{id}/snapshots` · `GET /snapshots/{id}?part=ir|state|nodes|edges|calls` · `POST /projects/{id}/dims` · `GET /sameas?base=&target=` · `POST /sameas/decide` · `PUT /projects/{id}/iface-ledger` · `POST /diffs` · `GET /diffs/{id}?part=diff|summary|events` · `POST /targets` · `POST /targets/{key}/jobs` · `POST /jobs/{id}/pause|resume|cancel` · `GET /targets/{key}/coverage|registry|panels|brief` · `PUT /targets/{key}/verdict` · `PUT /registry/{cluster}/status` · `POST /panels/{id}/complete` · `GET /projects/{id}/character|similar` · `GET /refs/{ref}` · `GET /snapshots/{id}/rule_hits` · `GET /precedents?diff_id=` · `POST /targets/{key}/refresh_roster` · `GET /meta/taxonomy|adapters|metrics`.

### 0.5.2 MCP 도구(hwax-risk)

`risk_get_snapshot(snapshot_id, part)` · `risk_get_diff(diff_id, part)` · `risk_get_registry(target_key)` · `risk_claims_for_ref(ref)` · `risk_submit_panel_result(panel_id, engine, decision_text, turns, report_id)`.

### 0.5.3 Settings(`config.py`)

`risk_review_enabled=false` · `risk_store_path='data/risk_review.sqlite'` · `risk_roster_domains`(15) · `risk_ecad_domains`(6) · `risk_adjacency`(JSON) · `risk_concurrency=2` · `risk_daily_panel_cap=24` · `risk_default_close_level='C2'` · `risk_carried_days=90` · `risk_panel_llm_cap=120` · `ra_admin_pat`(secrets) · `adh_api_key`(secrets) · `adh_team` · `adh_group`.
...
| 항목 | 값 |
|---|---|
| 패널 좌석 | 6(primary 4 + counter 1 + adversary 1), rounds 3, free_tools 1, tool_budget 3, modifiers 기본 `['toulmin']`(Tier A 는 `premortem` 추가 선택) |
| delib_opts 상한 | tools ≤6, apps ≤3, evidence ≤12항목. 엔진 클램프(PY `_resolve_opts` :464-467 / JS :128-130) source ≤120·tool ≤80·args ≤400·result ≤2000. 예산 11000 은 `result` 가 아니라 라인 `· [source · tool(args)] result` 전체 길이의 누적(PY :2164-2168 / JS :130-133)이며 초과 항목부터 통째 드롭. 포털 예산표(§5.6.1)는 라인 기준 합 10600 으로 드롭 0 을 보장 |
| 프롬프트 상한 | tool_inject 5000자, 지식카드 3500자/석, 자유조회 블록 3500자/석, role ≤2000자 |
| 저장 상한 | turns 60/대화, activity 60, RA rich_text 1900자 분할, seat_opinion say_excerpt ≤2000자·excerpt_for_rag ≤1500자 |
| E7 슬롯 | 좌석당 줄 ≤220자(참조 접두 포함), 좌석 합 ≤1100자, 항목 라인 상한 1400자(오버헤드 ≈171 + 프레이밍 ≤80 을 뺀 실효 ≥1100) |
| 예산 게이트 | 좌석 수·rounds 로 사전 산정한 LLM 호출이 `risk_panel_llm_cap`(120) 초과 예상이면 rounds=2 로 편성, 사후 초과는 `quality.flag=over_budget`(재실행 없음) |
| 재시도 | 패널 error → retry+1, retry > 2 → skipped(reason=engine_fail). `parse_retries` 하한 2 는 search_sources 사용 시에만(toulmin 무관) |
| 동시성 | risk_concurrency 2(타깃 다를 때만), 타깃당 패널 직렬, 일일 상한 24 |
| carried | ≤ risk_carried_days(90) |
| 완결 | C1 = 15 도메인 종결 ≥1 AND Tier A 3패널 done. C2 = 도메인별 종결 ≥ max(3, ceil(0.3·\|d\|)) AND strong 비율 ≥0.7 AND contested 표기 완료 AND 파싱 실패 0. C3 = 로스터 전원 종결 AND skipped ≤5% AND failed 0. RA 반영은 조건에 넣지 않고 external_sync 로 표기 |
| 패널 품질 목표 | 도구 사용률 ≥80%(귀속 가능 좌석), IR 인용률 ≥50%(name: 포함), 반대석 기각률 10~40%, 귀속 성공률 ≥95% |
| 패턴 승격 | candidate: 타깃 ≥3·project ≥2·expert ≥2. known: 큐레이터 승인 AND (confirmed ≥1 OR finding ≥5 across project ≥3). rule: n_labeled ≥10·precision ≥0.6·recall ≥0.5. predictor: n_labeled ≥50·project ≥15·CV R² ≥0.6 또는 AUROC ≥0.75 |
| 로스터 실측 | ≈350(xd 122 · sim 22 · cam 21 · rel 20 · soc 20 · disp 19 · mech 19 · pcb 19 · rf 19 · passive 18 · pwr 18 · sh 17 · mem 16 · std 8 · material 1), ECAD 부재 시 deferred ≈105 |
| Tier 패널 수 | 패널 수 = ceil(추가 좌석 / 5). 실측 ≈350: A 3 · B +20(누적 114석) · C +48(236석), 합 71. ECAD 부재(deferred ≈104): A 3 · B +14(85석) · C +33(161석), 합 50. §0.1.5 Tier 행·§6.4.1 표와 동일 수치 |
...
| §8 | 포털·MCP 통합 | 프론트·REST·MCP 파리티(L1/L2)·hwax-risk·무손상 변경 목록·반영 절차 |
| §9 | 단계 계획 | P0~P7 목표·산출물·의존·수치 통과 기준·리스크·'이 단계만으로 얻는 것' |
| §10 | 열린 질문·결정 필요 | 승인·보안·예산·어휘·ODB·공유 정책 결정 목록 |
...
|---|---|---|---|
| StepForge(`heax-step_forge`) | tree.json·graph.json·/parts REST 원문, MCP 읽기 도구(list_parts·list_interfaces·interface_graph·project_tree·part_mesh_map·mesh_report·job_status·inspect_report) | 읽기 전용(게이트웨이 rest_proxy heax 사이트 GET + MCP) | stepforge.db 무쓰기. `set_interface·confirm_interfaces·run_job` 은 사람이 명시 버튼으로만 실행(§2.10 'StepForge 에 반영') |
| DynaForge(`heax-kooremapper_mcp`) | `inspect_file`(meta.modelmeta 정본)·`report_summary/part_risk/findings/energy_flow/worst_cases`·전사 집계 3종(material_usage·section_contact_usage·corpus_summary) | 사용자 PAT 로 읽기(§2.13, 사용자 PAT 대리 발급 P0 실측·폴백) | `run_operation` 자동 실행 금지(modelmeta detect 는 사용자 실행 후 download_result 결과를 어댑터에 전달). 도구 시그니처·응답 형식 불변. KooRemapper C++ 확장(SECID 등) 범위 밖 |
...
| AIDataHub | doc_type 5종·의사 에이전트 `risk-review-memory`(§0.3.3), `agent_search·recommend_agents·hybrid_search·get_record` | REST `POST /api/records/import?external_source=hwax-portal` UPSERT, `create_doc_type·create_agent` 1회 | `bind_records_to_agent·patch_agent` 호출 금지. team/group 자동 채움 금지. e5 코사인 절대값 비교 금지 |
| 심의 엔진 2종(`deliberation.py`·`hwax-deliberate.js`) | chair_template `risk-review` 항목, 지정 반대석 `delib-baseline-defender`, 좌석 계약 상수, 앱 조건부 읽기 도구 목록, `_RISK_KEEP_TOOLS` 1줄 통과 | dict 항목·상수·조건 1줄 추가(§8.4), `check_chair_parity.py` 바이트 동일 검증 | 라운드 루프 본체·`_FREE_ALLOW` 전역·`_restore_role`·SSE 스키마·기존 8 chair·5 modifier·delib_opts 기존 키 의미 |
| HWAX Portal | 신규 패키지 `backend/app/risk/`(§0.4.1), 라우트 `/api/risk`, 프론트 `/risk-review/*`, sqlite 신규 파일 `data/risk_review.sqlite`, conv_store kind `risk-review` | `risk_review_enabled=false` 기본·플래그 조건부 라우터, conv_store 패턴(sqlite3+Lock+owner_sub) 복제 | 기존 라우트 6개·conv_store/token_store 스키마(kind 값 추가만)·`/api/agent/chat` 계약 |
...
| 산출물 | 형태 | 정의 절 |
|---|---|---|
| 과제 현황판 | rr_projects·소스 카드·스냅샷 목록·게이트 배너·character_seed·rule_hits | §2, §3, §8 |
| 스냅샷(rr_ir) | 불변 JSON + 노드/엣지 표 + mermaid, `GET /snapshots/{id}?part=ir\|state\|nodes\|edges\|calls` | §2 |
| 비교 화면(rr_diff) | base/target 선택 → SameAsResolver → GateBanner → DiffView(3층) + summary_text | §3 |
| 심사 타깃(rr_targets) | `snap:` 또는 `diff:` 단위 진행판·로스터·커버리지 히트맵·'미착석 N명(C3 미달)' 배지 | §6 |
| 패널 결정문 + risk_spec | RA `deliberation` 템플릿 보고서(패널마다), 대화(conv_store kind `risk-review`) | §4, §6 |
| 등록부(rr_registry) | cluster_key 병합 표·verdict 후보·사람 확정 `verdict_final` | §4 |
| 리스크 심사 보고서(통합) | 레벨 상승(C1/C2/C3) 시 RA 보고서 v1/v2/v3 | §4, §6 |
| 과제 성격 프로파일(character_profile) | facet 8 진술 + status seed→panel→confirmed, AIDataHub `project_character` | §4, §5 |
| 유사 과제·선례 | 계보·feature_vector kNN·hybrid_search·subject_key 회수, E5~E8 | §5, §7 |
| 학습 산출 | rr_delta_priors·rr_patterns·rr_rules(rule_hits)·rr_metrics·'루프 작동' 배지 | §7 |
...
| 두 엔진 파리티 | `CHAIR_ITEMS·CHAIR_ADVERSARY·RISK_SEAT_CONTRACT·제목` 바이트 동일(`check_chair_parity.py`), 파서·병합·저장은 포털 단일 구현, MCP 경로는 `/panels/{id}/complete` 로 되돌림(§8) |
| 애매동사 금지 | 각 절이 '사람 실행' 과 '코드 실행' 을 표로 구분한다(§2.10, §2.13, §6.7) |
| 메타 순수 리터럴 | whenToUse·CHAIR_ITEMS 문자열 연결 금지(§8) |
| 기존 기능 무손상 | additive 변경 목록(§8.4), 플래그 조건부 라우터, 별도 sqlite 파일, `_FREE_ALLOW` 전역 무수정 |
...
7. 사람 확정은 스냅샷 밖 원장(`rr_iface_ledger·rr_sameas·rr_part_keys`)에 두고 스냅샷 생성 시 재적용한다. 소스가 확정을 지워도 포털은 잃지 않는다.
...
| dyna_absent | dyna 소스가 없다(사용자 PAT 부재 포함). |
...
| `compare_reports` | IR 에 넣지 않음 | §3 결과층 diff 가 ckey 치환 후 같은 조인을 포털에서 수행한다(엔진 무수정). |
...
계약 정본은 `docs/design-risk-review/odb-adapter-contract.md` 이고 아래가 그 필드 표다. ODB hub 가 이 4도구를 게이트웨이 manifest `mcp:{}` 로 노출하면 `adapters/registry.py` 가 발견하고 `ecad_stub.py` 가 `ecad.py`(P7)로 교체된다. 그 전까지 스냅샷은 `missing.ecad_absent=true` 다.
...
| 7 | `manual` | intra·pair | `POST /sameas/decide {a, b, decision: confirm\|reject, scope}` → `rr_sameas` 기록. 다음 스냅샷(또는 '재해석 적용')에서 1단계로 흡수된다. | 1.0 · confirmed / rejected |
...
- 사람 게이트는 SameAsResolver(§8)에서 `pending`·`conflict`·`auto(fuzzy)` 를 표로 보여주고 confirm/reject 를 모아 `POST /sameas/decide` 로 보낸 뒤 '재해석 적용' 버튼이 `rr_snapshot_calls` 원문으로 ir_builder 를 다시 돌려(소스 재호출 없음) 새 스냅샷(`derived_from`=이전)을 만든다. 스냅샷은 바뀌지 않고 새로 생긴다.
...
- 사람 조작은 `POST /sameas/decide` 의 `decision ∈ merge_key | rename_key | confirm_key | unmerge_key` 로 받는다(`{ckey_from, ckey_into}` / `{ckey, display_name}` / `{ckey}` / `{ckey}`). `unmerge_key` 는 §5.9.2 의 병합 취소(`merged_into=NULL, status='confirmed'`)이며 아래 자동 승계 행에도 적용된다. 병합은 원장에만 쓰고 기존 스냅샷의 ckey 는 바뀌지 않으며, 조회·회수 코드는 항상 유효 ckey(merged_into 해석 후, `resolve_ckey()`)로 조인한다.
...
규칙. (1) `ck=` 선택자는 유효 ckey(merged_into 해석)로 푼다. 그래서 치수 정의는 ckey 가 같은 다른 과제로 '치수 정의 복사' 가 가능하다(§8 ProjectPage). (2) ref 가 사라지면 값은 null 이고 `null_reason` 이 남는다. 0 으로 두지 않는다. (3) 단위는 vocab 의 unit 이 정본이고 attr 단위와 다르면 정의 등록 시 `400 unit_mismatch` 다. (4) `POST /projects/{id}/dims` 는 정의를 저장만 하고 값은 다음 스냅샷(또는 '재해석 적용')에서 계산된다.
...
| dyna_pat_absent | WARNING | 사용자 PAT 없어 dyna 소스 생략 |
...
StepForge 는 재검출 시 `interfaces` 를 삭제·재삽입해 confirmed/manual 이 사라지고(job_worker.py:457-470), 재파싱은 노드 id 를 재부여한다. DynaForge 는 파트 별칭을 저장할 곳이 없다. 그래서 사람 확정 3종은 포털 원장에만 쓰고 스냅샷 생성 때 재적용한다.
...
| 행위 | 누가 | 어떻게 |
|---|---|---|
| 계면 kind/status 확정 | 사람 | ComparePage·SnapshotPage 에서 행 선택 → `PUT /projects/{id}/iface-ledger` |
| same-as confirm/reject, ckey 병합 | 사람 | SameAsResolver → `POST /sameas/decide` |
...
| 과제 계보 | `rr_projects.predecessor_project_id`(과제→과제, 사용자 지정, §5.2.2 A)가 RA `revision_of{project→project, acyclic}` 로 복제된다. StepForge `revision_of` 는 미구현이므로 포털이 유일한 원천이다. |

### 2.11.3 스냅샷 동결 절차(`POST /projects/{id}/snapshots {label, kinds[], report_ids?, detect_result_file_id?}` → 202 `{job_id}`, 백그라운드 잡, 예산 180 s, 진행은 `GET /projects/{id}` 의 `jobs[]` 폴링 — §8.1.4·§8.2.3)
...
3. dyna 캡처 — principal 에 사용자 PAT 가 없으면 `missing.dyna_absent` + `dyna_pat_absent` 로 건너뛴다. 있으면 `list_session_files` → `inspect_file` → (detect 결과 파일 지정 시) `download_result` → 전사 집계 3종.
...
- principal 은 사용자 PAT 를 가져야 한다. 경로는 §0.1.6 '사용자 PAT 대리 발급'(P0 실측 항목)이고, 실측 실패 시 폴백은 '사용자 브라우저 세션에서만 캡처(무인 배치는 dyna absent 강등)' 다. 게이트웨이 서비스 계정으로 private 세션을 여는 코드는 없다(P2 통과 기준 (8)).
...
- 골든 `tests/risk/golden/sif-e2e.ir.json` 은 재파싱·재검출 후의 sif-e2e(리프 3·엣지 2)이고 P1 통과 기준 (1)(2)(3)(8) 의 대상이다.
...
- 게이트 픽스처 5케이스(G1 fail·G2 fail·G4 fail·G5 partial·G6 blocked)에서 `{pass, blocking, effect}` 일치, G6 fail 시 `POST /diffs` 가 `409 gate_blocked`(P1 (4)).
...
|---|---|---|---|---|
| rr_state | rr_ir 1개(+ rr_rules active) | 스냅샷 동결 직후 자동(`POST /projects/{id}/snapshots` 의 잡 마지막 단계) | `rr_states(snapshot_id PK, state_json, feature_json, rule_hits_json, character_seed_json, summary_text, state_version, created_at)` | `sig:<key>` · `gate:G<n>` · `rule:<rule_id>` · `warn:<code>#<ref>` |
| rr_diff | rr_ir 2개(base·target) + 두 rr_state + pair 대응(§2 same-as 사다리) | `POST /diffs` 요청 시 동기 계산(노드 500·엣지 2000 에서 5 s 이내) | `rr_diffs(id PK, base_snapshot_id, target_snapshot_id, diff_json, comparability_json, stats_json, summary_text, summary_status, diff_version, created_at, UNIQUE(base,target))` + `rr_diff_events(diff_id, cid, code, change_kind, subject_key, ckeys_json, magnitude, unit, rel, confidence, design_relevant, unconfirmed, excluded_reason, text, PRIMARY KEY(diff_id, cid))` | `[c:<cid>]` · `[d:<name>]` · `gate:G7` |
...
| G2 sameas_pending | `same_as.status='pending'`(auto score < 0.9) 건수 + 같은 도메인 노드 2개가 한 클러스터에 든 conflict 수 | `= 0` | snap: `mark` — pending 노드는 dn 이 자기 nid 이고 results 오버레이 조인을 하지 않는다. pair: `block_semantic` — 의미층 이벤트를 만들지 않고 `semantic.blocked_by='G2'`, 구조·파라메트릭층은 대응 확정분만으로 만든다 | 불가(POST /sameas/decide 로 해소) |
...
| G6 unit_scale | tree.warnings `unit_mismatch` 건수 + mcad `unit_system != 'mm'` 이면 +1 + (dyna 가 있을 때) `dyna 모델 bbox 대각 / mcad bbox_world 대각` 이 `[0.95, 1.05]` 밖이면 +1(계산 불가면 `unknown`, 계수 0) + ecad `odb_get_board.units` 환산 실패면 +1(§2.12) | `= 0` | `block` — `blocked=true`. diff 생성(`POST /diffs`)은 409, 타깃 생성도 409. 사람이 소스를 고쳐 새 스냅샷을 만들어야 한다 | 불가 |
...
게이트 픽스처 5케이스(P1 통과 기준 (4)의 정의). `tests/risk/fixtures/gates/`.
...
| gate_f3_unit | tree.warnings `unit_mismatch` 1 | G6 fail, blocked=true, `POST /diffs` 409 |
...
`base_blocked`·`target_blocked` 는 두 rr_state 의 `blocked` 이고 어느 하나라도 true 면 `POST /diffs` 는 409 `{gates}` 로 끝나 diff 행이 생기지 않는다.
...
합성 픽스처 6종(P2 통과 기준 (1)의 정의). `tests/risk/fixtures/diff_pairs/`. 각 쌍은 golden IR 에 한 가지 변형만 준 것이고 기대 이벤트는 정확히 그 1건, 나머지 항목은 noise 또는 없음이다.
...
- `rr_diffs.diff_json` 은 전문이고 `rr_diff_events` 는 의미 이벤트 펼침 표(cid·code·change_kind·subject_key 인덱스)다. `GET /diffs/{id}?part=events` 는 펼침 표를, `?part=summary` 는 summary_text 와 comparability 만 돌려준다.
- `GET /refs/{ref}` 는 `sig:`·`c:`·`gate:`·`rule:` 도 해석해 원문 필드와 정규 표기를 돌려준다(§8).
...
| verdict · verdict_conditions | 판정 후보. 사람이 `PUT /targets/{key}/verdict` 로 `verdict_final` 확정(§6) | (8) |
...
parseSimSpec 복제다. 순서는 (1) 결정문 텍스트에서 마지막 ```json 펜스를 찾아 파싱, (2) 없으면 텍스트 끝에서 역방향으로 마지막 균형 중괄호 블록을 파싱, (3) `schema=='risk_spec'` 이 아니면 실패, (4) 실패는 null 이고 비치명이다 — 패널은 done 으로 남되 `rr_panels.risk_spec_parsed=false`·`quality.flag=spec_parse_failed`, finding 0건, C2 조건(미해결 파싱 실패 패널 0)에 걸려 해결 대상이 된다 — 해결은 §6.9 의 절차(사용자가 decision 텍스트를 보정해 `POST /panels/{id}/complete` 로 파서만 재실행, 또는 '제외' 표기)이고 LLM 재실행은 없다. 파서는 포털 단일 구현이고 MCP 경로(L2 오케스트레이터)는 결정문 텍스트를 `POST /panels/{id}/complete` 로 보내 같은 파서를 탄다(§8). 이것으로 gap_21 의 'P0 통과 기준이 P3 산출물에 의존' 모순을 닫는다 — P0 통과 기준 (6) 은 이 파서로 검증한다.
...
저장 시 finding·gain·cross_domain·character_statement·open_item 의 모든 cites 를 `rr_claim_refs(claim_uid, ref_type, ref, quote)` 에 펼친다. claim_uid 는 `<panel_id>#F1` 같은 전역 id 다. `GET /refs/{ref}` 와 `risk_claims_for_ref(ref)` 가 이 표를 조인해 "이 엣지에 대해 누가 뭐라 했나" 를 돌려준다. ref 가 `e:`·`p:` 이면 dn·ckey 로 확장해 다른 스냅샷의 같은 부품 claim 도 함께 돌려준다(§5).
...
- `tool_calls_n` 은 `status.step` 이 `'<key> 조회: <tool>'` 인 이벤트 수, `tool_calls_ok` 는 `evidence.source` 가 `'<key> · <tool>'` 인 이벤트 수다(gap_21 다섯 번째 항목의 규약). 입력 소스는 러너가 직접 읽은 SSE 스트림 또는 `POST /panels/{id}/complete` 의 `events[]`(§8.2.3, 브라우저 세션 폴백에서 프론트 PanelRunner 가 캡처)이고 두 소스에 같은 정규식(§6.7 7단계)을 쓴다. 둘 다 없으면 conv_store `meta.activity`(tool 붙은 status 만 보존, evidence 미보존 — routes.py:891-899)로 `tool_calls_n` 만 복원하고 `tool_calls_ok=null`·`used_tool=null` 로 둔다(§6.11 분모 제외). `used_tool = tool_calls_ok ≥ 1`(null 이면 null). 지정 도구(delib_opts.tools) 결과는 좌석 귀속이 불가하므로 세지 않는다. `activity_idx` 는 conv_store activity 에서 같은 좌석·같은 도구·같은 라운드의 항목을 시각 순으로 대응해 채우고, 대응 실패면 null.
...
- `status` 초기값 open. `verified`·`dismissed` 는 라벨(§7) 또는 사람 UI(`PUT /registry/{cluster}/status`)로만, `mitigated` 는 사람 UI 로만(같은 엔드포인트가 값 `mitigated` 를 받는다 — §8 에 표기), `superseded` 는 §4.8.
...
|---|---|---|---|---|
| 포털 sqlite `data/risk_review.sqlite`(`risk_store.py`) | 과제·소스·스냅샷·rr_ir 원본·호출 로그·원장(same-as·계면·ckey)·rr_state·rr_diff·타깃·로스터·커버리지·패널·잡·seat_opinion·finding·등록부·claim 역색인·성격 진술·계면 별칭·delta 선례·라벨·패턴·규칙·지표·큐레이션 큐·3자 id 매핑 — **ID 권위와 정본** | 조직 단위 의미 검색(AIDataHub 몫), 다중 관계 탐색(RA 몫) | stdlib sqlite3 + `threading.Lock`, 모든 쿼리 `owner_sub` | 포털 세션 또는 PAT(`principal_pat_or_session`) |
| ReportArchive KG | 과제·스냅샷·diff·판정·finding·전문가·성격 태그 사이의 **관계**와 enum/number/date/text 급 소형 속성, 기존 `part · model · failure_mode · defect · incident · test_run` 축과의 연결, 심의·통합 보고서 | 서술 본문(longtext 는 검색·임베딩 대상이 아님), 노드 수천 개 그래프 원본(속성에 JSON 타입 없음) | 부트스트랩 1회는 관리 REST(`/api/entity-types` · `/properties` · `/api/relation-types`), 인스턴스는 MCP 4종 `create_object · update_object · add_object_alias · link_objects`, 보고서는 `create_report_draft · update_report_draft`. RA 코드 무수정 | 시스템관리자 PAT(`ra_admin_pat`, 서버 secrets) — 보고서만 사용자 PAT |
| AIDataHub | 좌석 서술(risk_review_opinion)·패널 결정문(risk_review_panel)·과제 성격(project_character)·패턴 카드(risk_pattern_card)·스냅샷 다이제스트 표(design_snapshot_digest) — 섹션 단위 임베딩으로 **RAG 회수 대상** | 관계·커버리지 회계·IR 원본·유니크 제약이 필요한 것 | REST `POST /api/records/import?external_source=hwax-portal` + `_external_id` UPSERT(`adh_client.py`), `create_doc_type`·`create_agent` 각 1회 | `adh_api_key`(서버 secrets) |

원칙.

1. **ID 권위는 포털이다.** RA 는 `code=<포털 id>` 로, AIDataHub 는 `external_id_map(source='hwax-portal', external_id)` 로 포털 id 를 따라간다. 외부 id 는 포털 `rr_id_map` 과 각 행의 `ra_entity_id · adh_record_id` 컬럼에 되돌려 적는다.
2. **외부 반영은 비치명이고 완결 레벨과 분리된다.** RA·AIDataHub 가 없어도 C1~C3 는 도달하며, 반영 상태는 `rr_targets.external_sync_json{ra, adh}` 로 따로 보인다(§5.5.3). C1 조건에 'RA assessment 생성 완료' 를 넣지 않는다(gap_21 #7).
3. **원본은 한 곳에만 있다.** IR·diff·finding·성격 진술의 정본은 포털이고 RA·AIDataHub 는 투영이다. 투영이 어긋나면 포털에서 다시 밀어 넣는다(UPSERT 멱등).
...
## 5.2 포털 sqlite `data/risk_review.sqlite`

### 5.2.1 공통 규약

| 항목 | 규약 |
|---|---|
| 모듈 | `backend/app/risk/risk_store.py` 의 `RiskStore`. `conv_store.py` 와 같은 골격 — `sqlite3.connect(path, check_same_thread=False)`, `threading.Lock`, 생성자에서 `CREATE TABLE IF NOT EXISTS` 전부 실행, `PRAGMA journal_mode=WAL`, `PRAGMA foreign_keys=OFF`(외래키는 문자열 계약, DB 제약으로 강제하지 않는다) |
| 경로 | Settings `risk_store_path='data/risk_review.sqlite'`, `main.py` lifespan 에서 `app.state.risk_store` 생성(`risk_review_enabled=true` 일 때만) |
| id | uuid hex32(`conv_store._uid` 패턴). 예외는 자연키 — `target_key · coverage_key · cluster_key · ckey · alias_key · call_id · nid · eid · cid` |
...
| 소유권 | 모든 표에 `owner_sub TEXT NOT NULL`. 조회는 `WHERE owner_sub=?` 를 기본으로 하고 `visibility='org'` 행만 예외로 읽는다 |
| 상태 enum | `CHECK(status IN (...))` 로 DB 에서 막는다. 값 목록은 §0 정의를 그대로 쓴다 |
| 스키마 버전 | `PRAGMA user_version` 에 정수 버전. 마이그레이션은 `ALTER TABLE … ADD COLUMN` 과 인덱스 추가만 허용하고 컬럼 삭제·타입 변경은 하지 않는다(새 표를 만들고 복사) |
...
스냅샷 1건은 `ir_json` ≈0.4~0.9 MB, 호출 로그 gzip ≈0.2~0.5 MB, 펼침 표 ≈0.3 MB 다. 과제 50건·스냅샷 200건이면 ≈300 MB 로 sqlite 단일 파일 범위 안이다. 삭제는 하지 않고 `superseded_by` 로 닫는다. 백업은 conv_store 와 같은 파일 복사 절차(WAL 체크포인트 후)다.

## 5.3 ReportArchive KG(관리 REST 부트스트랩 1회·MCP 인스턴스 쓰기·RA 코드 무수정)

### 5.3.1 신규 축 6(POST /api/entity-types → POST /api/entity-types/{type_id}/properties)

RA 속성 타입은 `text · longtext · number · date · year · bool · enum · entity_ref · url` 9종뿐이고 키는 `^[a-z][a-z0-9_]*$`(≤48) 이며 JSON 형은 없다. 그래서 IR·diff 원본은 절대 넣지 않고 카운트·해시·enum·요약만 넣는다. `value` 는 축 안에서 대소문자 무시 유니크(≤255)이고 `code`(≤64) 는 비유니크이므로 `code=<포털 id>` 로 멱등을 잡고 `value` 에는 시각·source 를 넣어 충돌을 막는다.
...
기본 단위는 등록부 클러스터다. finding 원자는 `risk_finding` 으로 만들지 않고 `statement` 에 대표 claim 만 담는다(원자는 포털 `rr_findings` 와 AIDataHub 의견 레코드 §3 절에 있다). 축당 기대 인스턴스 수는 타깃 하나에 `assessment ≤350 · risk_finding ≤60` 이고 `expert` 는 로스터 규모(≈350)에서 멈춘다.
...
{"slug":"design_snapshot","label":"설계 스냅샷","icon":"snapshot","multi":false,"sort_order":110,
 "description":"HWAX 리스크 심사 — 소스 앱을 한 시점에 동결한 설계 IR 헤더(원본은 포털 rr_snapshots)","kind_class":"record"}
```
...
## 5.4 AIDataHub(REST import UPSERT·doc_type 5종·의사 에이전트 1)
...
```
POST {ADH_BASE}/api/records/import?external_source=hwax-portal
<인증 헤더는 adh_api_key 로, P0 (3) 에서 실측한 형식>
...
MCP `import_record` 는 `_external_id` 를 받지 못해 재실행 시 중복을 만들므로 포털 서버 코드(`adh_client.py`)는 REST 만 쓴다. 응답의 `record_id` 는 `rr_seat_opinions.adh_record_id · rr_panels(evidence_refs_json 의 panel 항목) · rr_projects.adh_character_record_id · rr_id_map` 에 적는다. 저장 직후 허브가 signature 임베딩을 계산하므로 별도 색인 호출은 없다.
...
결과(명시). 이 선택으로 **엔진의 좌석별 자동 `agent_search`(자기 agent_type) 로는 그 전문가의 이전 리스크 발언이 절대 회수되지 않는다.** 좌석 개인 기억은 오직 포털이 조립하는 E7 고정 슬롯(§5.6.3)으로만 좌석에 도달한다. 대안(레코드 `agents` 에 실 키를 넣되 `required_tags` 와 `retrieval_config` 는 건드리지 않음)은 §10 열린 질문 (8a) 로 넘긴다.
...
| (b) 좌석 개인 기억 | 1차 `rr_seat_opinions WHERE agent_key=? AND target_key<>?` 최신순, 2차 `agent_search('risk-review-memory', q='<과제코드> <summary_text 앞 200자>', mode='hybrid', required_tags=['hwax:expert:<agent_key>'], retrieval_config={top_k:3})` | 포털 `narrative.prior_evidence`, 패널 편성 직후 | E7(좌석당 줄 ≤220자) |
| (c) 유사 서술 | `hybrid_search(q=<summary_text 앞 300자>, tags=['hwax-risk-review','mechanism:<m>'])` top 5 + `tag_search(['char:'+t for t in character_seed])` | 포털, §7.3 3단계 | E5·E6 후보, '유사 과제' 탭 |
...
| 방향 | 키 | 비고 |
|---|---|---|
| 포털 → RA | `entity.code = <포털 id>`(project=rr_projects.id · snapshot_id · diff_id · opinion_id · `<target_key>#<cluster_key>` · agent_key · 성격 태그) | `value` 는 §5.3.1 규약, 재호출은 `created:false` |
| RA → 포털 | `properties.portal_*_id`(text 중복 보관) + `rr_id_map.ra_entity_id` | 서브그래프 결과에서 포털 화면으로 돌아오는 경로 |
| 포털 → AIDataHub | `external_id_map(source='hwax-portal', external_id=<§5.4.2 규약>)` | 재실행 UPSERT, 레코드 id 불변 |
| AIDataHub → 포털 | `tags hwax:project:<id> · hwax:target:<key> · hwax:snapshot:<id>` + `content.meta.portal.*` | `tag_search` 로 역조회 가능 |
| RA → AIDataHub | `assessment.record_id · risk_finding.opinion_record_id · design_snapshot(없음)` | 텍스트 id |
| AIDataHub → RA | `content.meta.ra.{project_entity_id, assessment_id, report_id}` + `tags hwax:project:` 의 포털 id 를 `rr_id_map` 으로 변환 | |
| 포털 내부 | `rr_coverage.opinion_id → rr_seat_opinions`, `rr_findings.opinion_id · panel_id`, `rr_registry.merged_json.member_finding_ids`, `rr_claim_refs.claim_uid → rr_findings.claim_uid | rr_character.id`, `rr_labels.finding_id`, `rr_patterns ← rr_rules.pattern_id` | 삭제 없음이라 댕글링은 `superseded_by` 로만 생긴다 |
| 대화·보고서 | `rr_panels.conv_id → conv_store(kind='risk-review')`, `rr_panels.report_id · rr_targets.report_ids_json → RA report`, `finding.cites tool:conv:<conv_id>#<idx> → conv_store activity` | |

### 5.5.2 `rr_id_map`

모든 외부 반영 성공 시 `INSERT OR REPLACE`. 한 포털 객체가 RA 와 AIDataHub 에 각각 있는지 한 줄로 보이고, RA 서브그래프 노드 id → 포털 객체 역변환(`ix_rr_id_map_ra`)과 AIDataHub 레코드 id → 포털 객체 역변환(`ix_rr_id_map_adh`)에 쓴다. 각 원본 표의 `ra_entity_id · adh_record_id` 컬럼은 조회 편의용 중복이고 정합 검사는 야간 잡이 `rr_id_map` 기준으로 한다.
...
| `pending → unavailable` | `ra_admin_pat`/`adh_api_key` 미설정, 또는 401/403, 또는 attempts ≥ 8. 사용자가 secrets 를 넣고 '재동기' 를 누르면 `pending(attempts=0)` 으로 되돌아간다 |
| 새 패널 종료 | `done` 이었어도 새 op 가 쌓이면 `pending` |

C1~C3 판정은 이 값을 읽지 않는다. 타깃 화면과 통합 보고서 헤더에 `RA 반영: done|pending(n)|unavailable · AIDataHub 반영: …` 로 표기만 한다(gap_21 #7). `unavailable` 상태에서도 다음 과제 브리프(E5~E8)는 포털 원장만으로 조립되므로 재사용 루프는 끊기지 않는다 — 끊기는 것은 (c) 유사 서술 경로와 KG 그래프 검색뿐이며 그 사실을 E0 에 `외부 검색 미가용` 한 줄로 적는다.

### 5.5.4 역참조 예

- RA 서브그래프에서 `risk_finding` 노드를 클릭 → `code` 를 `'#'` 로 나눠 `rr_registry(target_key, cluster_key)` → 화면 `/risk-review/targets/<key>#<cluster>`.
...
`narrative.prior_evidence(target, panel)` 가 다음 패널의 `delib_opts.evidence`(≤12 항목, 엔진 클램프 source ≤120·tool ≤80·args ≤400·result ≤2000, 예산 11000 초과 항목은 엔진이 통째 드롭) 를 조립한다. 엔진 상한에 기대지 않고 포털이 항목별 상한을 먼저 강제해 **드롭 0** 을 보장한다(P5 통과 기준 (1)). 측정 단위는 `result` 문자열이 아니라 엔진이 실제로 누적하는 **라인** `· [{source} · {tool}({args})] {result}` 의 `len` 이다(deliberation.py :2162-2168 `_line = f"· [{_src}{_meta}] {_res}"`, hwax-deliberate.js :128-133 동일 식). 그래서 항목마다 source·tool·args·구두점 오버헤드(§5.6.1 규칙)가 붙고 예산표는 그것을 포함해 합 10600 으로 잡는다. P3 (1)·P5 (1) 에서 엔진 로그의 evidence 채택 수가 보낸 항목 수와 같은지(드롭 0) 확인한다.
...
| E1 | `rr_diff`/`rr_state` | pair 면 `summary_text`(§3.4.2 순서 그대로, 실효 상한까지 줄 경계 절단 — 뒤 섹션 `[씨앗]`·`[재료]` 부터 `…(n줄 생략)`, 전문은 `GET /diffs/{id}?part=summary` 이고 `[의미]`·`[치수]`·`[결과]` 는 E2·E3·E4 가 표로 따로 싣는다), snap 이면 rr_state 요약(counts·worst_interference·thin_clearances·orphans·rollup_top, 줄마다 `[sig:]`·`[p:]`·`[e:]`) | rr_diffs.summary_text·rr_states | 1900 | 없음(필수) |
...
def prior_evidence(target, panel):
    ...
    lineage = ra_or_portal_lineage(target.project_id, hops=3)    # RA revision_of 우선, 없으면 rr_projects.predecessor 체인
...
2. 포털에 없으면 AIDataHub `agent_search('risk-review-memory', q='<과제코드> <summary_text 앞 200자>', mode='hybrid', required_tags=['hwax:expert:<agent_key>'], retrieval_config={'top_k':3})` 의 첫 적중 섹션 원문(같은 220자 줄 규칙, 발췌 ≈125자. 참조는 그 레코드 `content.meta.portal.opinion_id` 로 재구성한 `narr:`). RA·AIDataHub 미가용이면 이 단계는 건너뛴다.
...
| 호출 | 입력 | 출력 | 실리는 곳 |
|---|---|---|---|
| `GET /api/risk/projects/{id}/similar?k=5` | 과제의 최신 스냅샷 feature_vector·character_seed·summary_text | `{lineage:[{project_id, code, hops, relation}], vector:[{project_id, snapshot_id, cosine, rank, top_features:[3]}], text:[{record_id, project_id, rank, section_id}], subject:[{subject_key, n_registry, n_verified}], merged:[{project_id, score, paths:[…]}], corpus_n}` — `corpus_n<5` 면 `vector:[]`, reason 병기 | ProjectPage '유사 과제' 탭(경로별로 따로 보여주고 섞지 않는다), E5·E6 후보 |
| `GET /api/risk/precedents?diff_id=` | diff 의 semantic 이벤트 | `{delta_priors:[{change_kind, mechanism, mechanism_detail, n_raised, n_targets, n_verified, n_dismissed}], clusters:[{reg_ref, target_key, project_code, cluster_key, subject_names, severity, status, support, claim, path}], rule_hits:[…], pattern_candidates:[{pattern_id, status, n_projects, precision}]}` | ComparePage '선례' 패널, E5·E8 |
| `GET /api/risk/targets/{key}/brief?tier=&exclude=E6,E8` | 제외할 항목 키 | 조립된 evidence 배열(§5.6) 미리보기 | RecallPreview — **항목 제외만 가능, 추가·편집 불가**(P1 을 UI 로 보장) |
| `hwax-risk` `risk_get_registry(target_key)` · `risk_claims_for_ref(ref)` | | 등록부 행 / ref 역색인 결과 | 다른 심의 좌석의 자유조회(P5) |

응답에는 왜 유사한지의 분해(세 경로 점수·공유 태그·가장 가까운 feature 3개)를 함께 내 사람이 판단한다. 절대 코사인 임계는 어디에도 없다.
...
5. **RA.** 선례 인용 자체는 새 관계로 만들지 않는다(12관계 고정). 후속 finding 의 `raised_by` 만 만들고 선례 링크는 포털 `precedent_refs` 로만 보관한다.
...
- 사람 확정(`confirmed`)은 SnapshotPage 노드 표에서 한다. 병합은 `status=merged, merged_into=<대표>` 이고 대표 쪽 `aliases_json` 에 흡수한다. 병합 취소(`POST /sameas/decide {decision:'unmerge_key', ckey}`, §2.7.3 자동 승계 행 포함)는 새 행을 만들지 않고 `merged_into=NULL, status=confirmed, merge_evidence_json=NULL` 로 되돌린다.
...
하지 않는 것. 설계 변경 실행, 시험 수행, StepForge `set_interface/confirm_interfaces` 나 DynaForge `run_job/run_operation` 의 자동 호출, verdict 자동 승인, 보고서 자동 배포. 좌석은 읽기 전용 도구만 자유조회하고(§6.5), 사람 확정은 포털 원장(§2)과 verdict UI(§6.9)에서만 일어난다.
...
- 로스터 갱신은 사용자가 `POST /api/risk/targets/{key}/refresh_roster` 를 누를 때만 일어난다. `list_agents` 에 새로 생긴 키만 `pending` 행으로 추가하고 `rank_in_domain` 은 기존 최대값 다음부터 붙인다(이미 편성된 패널의 결정론을 깨지 않는다). 기존 행(종결·비종결)은 불변이다.
- 실측 규모 ≈350(xd 122 · sim 22 · cam 21 · rel 20 · soc 20 · disp 19 · mech 19 · pcb 19 · rf 19 · passive 18 · pwr 18 · sh 17 · mem 16 · std 8 · material 1). ECAD 부재 시 deferred = 110 − 6 = 104 ≈105 석이고 pending ≈245 석이다. 이 수치는 `POST /targets` 응답 `{roster_size, deferred, tier_plan, cost_estimate}` 로 사용자에게 먼저 보이고 동의를 받는다.
...
5. `rr_panels` 행 생성 — `{id, target_key, panel_no(타깃 내 1부터), tier, seats_json, status='planned', engine='web', tool_mode='tools', budget_json(§6.10), created_at}`. `seats_json = [{key, domain, origin, rank_in_domain}]` 이고 **role 은 저장하지 않는다**(엔진이 복원, 계약은 엔진 상수 — §6.5). 같은 원장 상태·같은 Settings 이면 같은 seats_json 이 나온다(결정론 테스트 `tests/risk/test_planner.py`).
...
_RISK_READ_TOOLS = {
 ...
 "hwax-risk": ("risk_get_snapshot","risk_get_diff","risk_get_registry","risk_claims_for_ref"),   # P5
...
4. `check_chair_parity.py` 가 `docs/design-risk-review/seat-contract.v1.json` 을 직렬화한 문자열과 PY/JS 상수를 바이트 비교한다(원천은 JSON, 상수는 손으로 옮긴 리터럴 — 메타 순수 리터럴 규칙).
...
파서 `parse_risk_spec`(펜스 우선 → 마지막 균형 중괄호 → 실패 null 비치명)은 포털 단일 구현이고 P0 산출물이다(§4). 실패는 패널 `risk_spec_parsed=false`·`quality.flag=spec_parse_failed` 로 남고 LLM 재실행은 하지 않는다.
...
1. **잡 선택·패널 잠금** — `rr_jobs.state ∈ {queued, running}` 잡을 집고(같은 타깃에 `running` 패널이 있으면 건너뜀 → 타깃당 직렬), 일일 상한을 검사한 뒤 `planner.plan_next_panel` 로 다음 패널을 만든다. `rr_panels.status='running'`, 좌석 `rr_coverage.status='running'`, `started_at`.
...
3. **대화 생성** — `POST /api/agent/conversations {kind:'risk-review', title:'[리스크심사] {과제} P{n}'}` 를 타깃 owner 명의로 만든다(conv_store 스키마 무수정, kind 값 추가만). `/deliberate` 목록은 `serverKind='deliberation'` 필터라 0건 노출.
4. **principal·PAT** `[gap 수정 — 미확인 함수 확정 기술 제거]` — 두 모드 중 P0 통과 기준 (9) 실측 결과로 하나를 확정한다. (a) 대리 발급 모드 — 타깃 생성 시 저장한 owner `principal_json` 으로 패널마다 사용자 PAT 를 새로 발급하는 함수가 존재하고(이름·클레임·만료는 실측 기록), 게이트웨이의 사용자 스코프 위임이 그 PAT 로도 걸리면 무인 배치가 가능하다. (b) 브라우저 세션 모드 — 그런 함수가 없으면 패널은 사용자가 TargetPage 의 PanelRunner 를 열어 둔 브라우저 세션(세션 쿠키 principal)에서만 순차 실행되고, 무인 배치(야간 큐)는 DynaForge 도구를 apps·tools 에서 빼고 서비스 PAT 로 돌리며 패널 `quality.pat_degraded=true`·타깃 `missing.dyna_absent` 강등을 기록한다. 어느 모드든 SSE `warning(_pat_degraded)` 가 오면 `quality.pat_degraded=true`.
...
6. **루프백 호출** — 발급/세션 principal 로 포털 자신의 `POST /api/agent/chat` 을 호출한다(conv_store 저장·SSE 세마포어 슬롯·turns[:60]·activity[:60] 캡을 기존 코드 그대로 사용, 배치 점유 슬롯 ≤2).
7. **SSE 캡처(스키마 변경 없음)** `[gap 수정 — status.persona 필드 부재]` — status 이벤트 스키마는 `{step, tool, detail?, personas?, tools_used?}` 이고 좌석 키는 문자열에만 실린다(:2304-2308). 러너는 다음 규약으로 좌석을 귀속한다. 브라우저 세션 폴백(§8.2.4)에서는 러너가 SSE 를 보지 못하므로 프론트 PanelRunner 가 같은 스트림을 읽으며 `{kind: status|evidence|personas, step?, tool?, source?, personas?}` 압축 로그 `events[]`(≤400건)를 모아 `POST /panels/{id}/complete` 에 실어 보내고, `narrative.extract_seat_opinion` 은 SSE 스트림과 `events[]` 둘 다에 아래 정규식을 똑같이 적용한다(입력 소스만 다르다, §8.2.3).
...
8. **파싱·추출** — `parse_risk_spec(decision.text)` → `rr_findings`(finding·gain, cites 해석·dangling·feature_snapshot·evidence_grade·precedent 부여는 §4) · `rr_character(status='panel')` · `rr_seat_opinions`(좌석 5 + adversary 1, adversary 는 원장 미집계지만 의견은 저장) · `rr_claim_refs`. 파싱 실패면 findings 없이 좌석 의견만 저장한다.
...
11. **외부 반영(비치명·재시도 큐)** — RA `update_report_draft` tags 병합(`['심의','chat-deliberation','risk-review','hwax:target:<key>','hwax:panel:<no>']`, `get_report` 후 병합, 사용자 PAT), assessment/risk_finding/exhibits `create_object`+`link_objects`(관리자 PAT), AIDataHub REST import(opinion·panel, `_external_id` UPSERT). 결과는 `rr_targets.external_sync_json{ra: pending|done|unavailable, adh: pending|done|unavailable}` 로만 표기하고 완결 판정(§6.9)에는 넣지 않는다.
...
### 6.8.3 회계 불변식(`tests/risk/test_coverage_sm.py`) `[gap 수정 — extra_seats 정의 강화]`
...
| 최종 verdict | 사람이 `PUT /api/risk/targets/{key}/verdict {verdict, note}` 로 `verdict_final` 확정(등록부 집계는 후보, 자동 승인 없음) | RA assessment.verdict 갱신(external_sync) |

- `spec_parse_failed` 해결 = 사용자가 decision 텍스트를 보정해 `POST /api/risk/panels/{id}/complete` 로 재제출(파서만 재실행, LLM 재실행 아님) 또는 '제외' 표기(그 패널 좌석은 `done_weak` 로 강등). 재실행으로 C2 를 뚫지 않는다.
- `risk_default_close_level='C2'` — Tier B 잡이 끝나고 C2 가 충족되면 `C2(closed)` 로 마감하고 미착석 도메인·미소진 인원을 통합 보고서 minutes 에 기록한다. Tier C 는 `POST /targets/{key}/jobs {tier:'C', consent:true}` 명시 승인으로만 시작한다.
...
`hwax-deliberate.js` 에는 좌석 도구 호출 경로(free_tools·tools·apps)도 읽기 전용 강제도 없다. 따라서 MCP 경로 패널은 두 등급으로만 원장에 기록한다.

| 경로 | 무엇 | 원장 기록 | 커버리지 계수 |
|---|---|---|---|
| L1 단발 | `hwax-deliberate chairTemplate:'risk-review'`(evidence·personas 를 호출자가 줌) | 원장 미연동. `/panels/{id}/complete` 로 보내면 아래 L2 규칙 | — |
| L2 오케스트레이터 | `hwax-risk-review.js` 가 포털 `GET /targets/{key}/brief?tier=` 로 패널 목록·delib_opts 초안을 받아 `hwax-deliberate.js` 를 자식 호출하고 결과(decision·turns·report_id)를 `POST /api/risk/panels/{id}/complete {engine:'mcp', …}` 로 되돌림 | `engine='mcp'`, `tool_mode='evidence_only'`, `tool_calls_n=null` | 좌석은 `cited_refs≠∅` 이면 `done`, 아니면 `done_weak`. C2 strong 비율 계산에서 분모·분자 모두 제외. `used_tool=null` |
| 웹 폴백(브라우저 세션) | 프론트 PanelRunner 가 `/api/agent/chat` SSE 를 직접 열고 `POST /panels/{id}/complete {engine:'web', events[]}` 로 되돌림(§8.2.4) | `engine='web'`, `tool_mode='tools'`, `tool_calls_n/ok`·`extra_seats` 는 `events[]` 에서 §6.7 7단계 정규식으로 계산. `events[]` 부재 시 conv_store activity 로 `tool_calls_n` 만 복원, `tool_calls_ok=null`·`used_tool=null` | `used_tool` 이 null 인 좌석은 MCP evidence_only 와 같은 규칙 — `cited_refs≠∅` 이면 `done`, 아니면 `done_weak`, C2 strong 비율 분모·분자 제외. `events[]` 가 있으면 웹 러너 패널과 같은 계수 |
...
- Tier A 대표 패널과 무인 배치(야간 큐)는 웹 엔진 전용이다. MCP 경로는 사용자가 Claude Code 에서 직접 돌리는 보충 회차로만 쓰고 진행판에 `engine=mcp` 배지가 붙는다.
- 파서·병합·저장은 포털 한 곳(`/panels/{id}/complete` → `parse_risk_spec` → §6.7 8~12단계)에서만 하고 JS 는 파싱하지 않는다. 워크플로 스크립트에서 포털 REST fetch 가 불가하면 포털 MCP 앱 `hwax-risk` 의 `risk_submit_panel_result(panel_id, engine, decision_text, turns, report_id)` 로 대체한다(§8).
- 이 비대칭은 decision-table.md 파리티 표에 "JS: evidence-only(N/A: free_tools·tools·apps), RISK_SEAT_CONTRACT 동일 바이트" 로 기록한다.

## 6.12 이 절의 테스트 목록(`backend/tests/risk/`)
...
## 7.1 리스크 택소노미 v1(`docs/design-risk-review/taxonomy.v1.json`, 로더 `taxonomy.py`, 프론트는 `GET /api/risk/meta/taxonomy`)
...
승격은 항상 사람 승인이고 자동 활성화는 없다. 승격 결정 엔드포인트는 `PUT /api/risk/registry/{cluster}/status` 와 같은 패턴의 큐레이션 결정 API 로 §8 이 P6 에서 정의하며, 백테스트 미달(`precision <0.6 · recall <0.5 · n<10`)이면 422 를 돌려주고 상태를 바꾸지 않는다(P6 (4)). `rule → predictor` 는 게이트 미충족 시 학습 코드가 0줄이어야 한다(P6 (7)).
...
이 절은 §2~§7 이 정의한 것을 포털 코드·엔진 코드·게이트웨이 설정에 어떻게 얹는지를 파일 단위로 적는다. 이름은 전부 §0 정본을 쓴다. 프론트 라우트 접두는 `/risk-review`, REST 접두는 `/api/risk`, 기능 플래그는 Settings `risk_review_enabled=false` 이며 플래그가 꺼져 있으면 라우터 자체를 등록하지 않아 기존 앱에 미치는 영향이 0 이다. 아래 줄 번호는 2026-08-31 기준 실파일에서 확인한 것이고(deliberation.py · routes.py · hwax-deliberate.js · rest_proxy.py · gateway_config.json), 구현 시점에 다시 확인한다.

## 8.1 프론트(React/Vite, 추가만)

### 8.1.1 `delibTaxonomy.ts` — Job 'risk-review' 8행째

`frontend/src/components/chat/delibTaxonomy.ts` 는 `JobId` 유니언(:5) · `JOBS` 배열(:29~37, 7행) · `JOB_BY_ID`(Record) · `JOB_ROUTING`(Record, :66) · `suggestJob`(:94) 로 이루어져 있고, `Record<JobId, …>` 가 키를 강제하므로 `JobId` 에 값을 더하고 `JOB_ROUTING` 에 항목을 빠뜨리면 `tsc -b` 가 실패한다. 이것이 누락 방지 장치다.

변경 3곳(기존 7행 객체의 텍스트는 1바이트도 바꾸지 않는다).

```ts
// (1) JobId 유니언 끝에 추가
export type JobId = /* 기존 7개 */ | 'risk-review';

// (2) JOBS 배열 8행째(judge 그룹 끝, 'sim-plan' 앞이 아니라 배열 마지막 원소 뒤에 append)
...
`suggestJob(usedTools)` 에는 분기 1개를 기존 분기들 **뒤에** append 한다 — `usedTools` 에 `interface_graph · list_interfaces · compare_reports · risk_get_diff · risk_get_registry` 중 하나라도 있으면 `{id:'risk-review', why:'설계 그래프·리포트 비교 도구를 썼습니다'}`. 기존 분기가 먼저 매치되면 기존 결과가 그대로 나온다(회귀 0). 카피 규칙은 method-menu decision-table.md 를 따른다 — 상황이 헤드라인(`when`), 방법론이 서브(`engine`), 애매동사 금지(`note` 의 "실행이 아니라 판정 문서까지가 산출").

`HandoffBrief.tsx`·`DeliberatePage.tsx`·`StartPicker.tsx` 는 `JOBS` 를 순회해 카드를 그리므로 코드 변경 없이 8번째 카드가 나타난다. 챗 핸드오프(`startHandoff`, HandoffBrief.tsx:82)에서 이 Job 을 고르면 `delib_opts.chair_template='risk-review'` 와 대화 activity 원천이 evidence 로 실려 **원장 미연동 단발 심사(L1)** 가 된다. 이것이 P0 만으로 얻는 기능이다.

### 8.1.2 ConvKind 와 대화 저장

- `frontend/src/api/conversations.api.ts:6` — `export type ConvKind = 'chat' | 'deliberation' | 'risk-review';`.
- 백엔드 `backend/app/agent/routes.py:129` — `ConvCreate.kind: Literal["chat", "deliberation", "risk-review"]`. conv_store 의 `kind` 컬럼은 CHECK 없는 TEXT(conv_store.py:37)라 DDL 변경이 없다.
- `state/ChatContext.tsx:293` 이 서버 대화를 `m.kind === serverKind` 로 거르므로 `/deliberate`(serverKind 'deliberation')에는 리스크 패널 대화가 0건 노출된다. P3 통과 기준 (5) 가 이것을 검증한다.
- 패널 대화 열람은 `TargetPage` 안에서 `<ChatProvider storagePrefix="hwax.risk" sendPrefix="/심의 " serverKind="risk-review">` 로 감싼 `DelibView`·`ActivityPanel`(`components/chat/`) 을 재사용한다. `Composer` 는 장착하지 않는다 — 패널 대화는 러너가 만들고 사람은 읽기만 한다. 대화 제목 접두 `[리스크심사] {과제} P{n}` 은 §0.2.2 규약이다.

### 8.1.3 라우트 4개와 내비게이션

`frontend/src/App.tsx` 의 `createBrowserRouter([...])`(:14) 에 기존 항목과 같은 모양(`<ProtectedRoute><AppShell>…</AppShell></ProtectedRoute>`)으로 객체 4개를 `{ path: '*' }` 앞에 추가한다.

| path | element | 비고 |
|---|---|---|
| `/risk-review` | `RiskHomePage` | 메뉴 진입 |
| `/risk-review/projects/:id` | `ProjectPage` | `?snapshot=<snapshot_id>` 쿼리가 있으면 같은 화면 안에 `SnapshotPage` 를 렌더한다(라우트를 늘리지 않는다) |
| `/risk-review/compare` | `ComparePage` | `?base=&target=` 쿼리로 진입 상태 복원 |
| `/risk-review/targets/:key` | `TargetPage` | `:key` 는 `snap:<id>` 또는 `diff:<id>` 를 URL 인코딩한 값 |

`frontend/src/components/layout/AppHeader.tsx`(§0.4.4) 에 NavLink '리스크 심사' → `/risk-review` 1개를 추가한다. 링크는 `useRiskEnabled()` 훅이 `GET /api/risk/meta/adapters` 를 세션당 1회 호출해 200 이면 보이고 404(플래그 off)면 숨긴다 — 플래그가 꺼진 배포에서는 헤더도 종전과 같다.

### 8.1.4 API 클라이언트 `api/risk.api.ts`(신규)

기존 `apiFetch` 를 재사용하고 §0.5.1 의 REST 를 1:1 로 감싼다. SSE 는 쓰지 않는다. 진행 상태는 폴링으로 읽는다 — 스냅샷 잡은 `GET /api/risk/projects/{id}`(응답의 `jobs[]`), 배치 잡은 `GET /api/risk/targets/{key}/coverage`(응답의 `job`), 주기 5 s, 화면을 떠나면 중단. 새 엔드포인트 이름을 만들지 않는다. 오류 규약은 기존과 같다(401 → 로그인, 404 → 소유자 불일치 또는 플래그 off, 409 → 게이트/중복, 422 → 검증).

### 8.1.5 페이지·컴포넌트(`frontend/src/pages/risk/`)

| 화면 | 구역(위→아래) | 호출 API | 쓰기 행위 |
|---|---|---|---|
| `RiskHomePage` | 과제 카드 그리드(code · name · stage · 소스 3 아이콘 상태 · 최근 스냅샷 시각 · 열린 타깃 수 · 커버리지 % · level 배지) · 상단 탭 과제/비교(diff 목록)/타깃/보고서 · '과제 등록' 폼(code ≤40 · name ≤200 · stage 정규식 `(pre\|dv\|pv\|pra\|mp)([123r])?` · predecessor_project_id 선택 → RA `revision_of` · adh_scope{team, group} 사용자 입력 1회, 자동 채움 없음) | `GET/POST /projects` | 없음 |
| `ProjectPage` | 헤더(과제 · 계보 링크 · external_sync 배지) · 소스 카드 3장(mcad/dyna/ecad — `unlinked → linked → unreachable`, 연결 시 probe 결과 표시; ecad 는 어댑터 미발견이면 '연결 대기(스텁)') · '스냅샷 동결' 버튼(kinds 체크 · report_ids · detect_result_file_id 입력) → 잡 진행바 → 완료 시 `?snapshot=` 로 `SnapshotPage` · 스냅샷 목록 · dims 정의 편집(`rr_dim_defs`) · iface-ledger 편집 · 성격 프로파일(L0 seed · L2 panel · confirmed 3층 나란히, P5 에서 confirm 버튼) · 유사 과제(출처별 top-k, 섞지 않음) | `GET /projects/{id}` · `POST /projects/{id}/sources` · `POST /projects/{id}/snapshots`(202) · `POST /projects/{id}/dims` · `PUT /projects/{id}/iface-ledger` · `GET /projects/{id}/character` · `GET /projects/{id}/similar` · `GET /meta/adapters`(소스 선택지 `choices[]`) | 없음. `volume`/`material` 이 전부 null 이면 배너 '재파싱 필요' 와 StepForge 앱 링크(기존 `/launch/:systemId`)만 보여주고 포털이 `run_job` 을 호출하지 않는다 |
| `SnapshotPage` | 개요(ir_hash · 소스 · captured_at · counts · missing 플래그) · 게이트 표(G1~G7, `GateBanner`) · 노드 표(nid · name · ckey · dn · geom_fp · flags, 검색) · 엣지 표(eid · kind · status · min_gap · penetration_depth 하한 표기) · mermaid(StepForge EDGE_STYLE 규약, 300 엣지 초과 시 상위 서브어셈블리만) · dims_named 표 · rule_hits 표(`{rule, severity, pass, found, why_it_matters, fix_hint}`) · character_seed 칩 · warnings · 호출 로그 목록(`tool:<call_id>`) · '단발 심사 열기'(startHandoff 재사용, P1) · '타깃 만들기'(`POST /targets {kind:'snap'}`) | `GET /snapshots/{id}?part=ir\|state\|nodes\|edges\|calls` · `GET /snapshots/{id}/rule_hits` · `GET /refs/{ref}` | 없음 |
| `ComparePage` | base/target 선택(과제 → 스냅샷, 같은 과제 또는 계보 과제 우선 표시) · `SameAsResolver`(자동 매칭 표 method · score · status, 행별 confirmed/rejected, '자동 매칭 전부 수용' 은 score ≥ 0.9 만 일괄 confirmed 로 두고 `decided_by=owner_sub` 기록) · `GateBanner`(G6 fail → `blocked=true` 로 'diff 생성' 비활성, G2 fail → '의미층 차단' 표기, 나머지 표기 강제) · 'diff 생성' · `DiffView` 3층(구조 · 파라메트릭 · 의미, before/after mermaid, `summary_text` 각 줄 `[c:]` 각주 클릭 → 항목 하이라이트, 선례 패널 `GET /precedents?diff_id=` P5) · '타깃 만들기'(`POST /targets {kind:'diff', consent:true}`) | `GET /sameas?base=&target=` · `POST /sameas/decide` · `POST /diffs`(409 `{gates}`) · `GET /diffs/{id}?part=diff\|summary\|events` · `GET /precedents` · `POST /targets` | 없음(StepForge `set_interface/confirm_interfaces` 호출 없음 — 확정은 포털 원장에만) |
| `TargetPage` | 헤더(target_key · 과제 · ir_hash · level `C0…C3`/`C2(closed)` · verdict_final · `external_sync{ra, adh}` 배지 · superseded_by 링크 · **'미착석 N명(C3 미달)' 배지 상시**) · `CoverageHeatmap`(행 = 15 도메인, 열 = 커버리지 10 상태 카운트, 셀 클릭 → 좌석 목록 · opinion 링크 · `carried → pending` 되돌리기 · `skipped(reason)` 입력) · `PanelRunner`(Tier A/B/C · modifiers · user_memo · 예상 패널 수 · LLM 호출 추정 · 시작/일시정지/재개/취소 · 일일 상한 잔량 · concurrency ≤2) · 패널 목록(P{n} · engine `web\|mcp` · tool_mode · status · quality.flag 칩 · 대화 열기(DelibView) · RA 보고서 링크) · 등록부 표(`cluster_key` · mechanism · subject · severity · judgement · evidence_grade · precedent · support/contested · status, 필터 severity/domain/direction/status, 행 클릭 → cites → `GET /refs/{ref}` 팝오버, status 변경) · verdict 확정(후보 표시 → 사람이 `verdict_final` 확정, 자동 승인 없음) · `RecallPreview`(`GET /brief?tier=` 의 E0~E9 를 출처 태그와 함께, **항목 제외만 가능·추가 불가**) · 통합 보고서 링크(v1/v2/v3 RA report_id) · '로스터 갱신' | `GET /targets/{key}/coverage\|registry\|panels\|brief` · `POST /targets/{key}/jobs` · `POST /jobs/{id}/pause\|resume\|cancel` · `PUT /targets/{key}/verdict` · `PUT /registry/{cluster}/status` · `POST /targets/{key}/refresh_roster` · `GET /refs/{ref}` | 없음(원장 상태 변경은 전부 포털 sqlite) |

부품 `pages/risk/components/{SameAsResolver, GateBanner, DiffView, PanelRunner, RecallPreview, CoverageHeatmap}.tsx` 는 위 표의 구역과 1:1 이다. 그래프 렌더는 기존 `components/chat/renderers` 의 mermaid 경로를 재사용한다. 지표 차트(P6 `GET /meta/metrics`)는 dataviz 규약을 따르고 TargetPage 하단 '품질' 카드에 둔다.

### 8.1.6 화면 상태 어휘(배지 = 저장 값과 같은 문자열)
...
| 패널 | `planned · running · done · error` + `engine web\|mcp` + `tool_mode tools\|evidence_only` + `quality.flag[]` |
| 배치 잡 | `queued · running · paused(reason) · cancelling · cancelled · completed · failed` |
| external_sync | `ra: pending\|done\|unavailable` · `adh: pending\|done\|unavailable` |
| 등록부 | `open · verified · dismissed · mitigated · superseded` |

### 8.1.7 헌법이 화면에 나타나는 자리
...
- 포털 화면에는 소스 앱 쓰기 버튼이 없다. StepForge parse/detect · DynaForge `run_operation(modelmeta, detect)` 는 각 앱 화면에서 사용자가 실행하고, 포털은 그 산출(`detect_result_file_id` · report_ids)을 **지정**만 받는다.
- verdict 는 등록부 집계로 '후보' 만 보여주고 사람이 확정한다. 자동 승인·자동 배포 없음.

## 8.2 백엔드(FastAPI, 신규 패키지 `backend/app/risk/`)

### 8.2.1 패키지 구성과 등록 진입점

파일 목록은 §0.4.1 그대로다. 기존 파일 변경은 §8.4 목록뿐이다.

```python
# backend/app/risk/__init__.py — risk_review_enabled 조건부 등록 진입점
def register(app, settings) -> None:
    if not settings.risk_review_enabled:
        return                                   # 라우터·스토어·러너 전부 미생성
    from .risk_store import RiskStore
    from .runner import RiskRunner
    from .routes import router
    app.state.risk_store = RiskStore(settings)   # data/risk_review.sqlite, CREATE TABLE IF NOT EXISTS
    app.state.risk_runner = RiskRunner(app, settings)
    app.include_router(router)                   # prefix /api/risk
```

`backend/app/main.py` 변경은 4줄이다 — import 1줄, 기존 `app.include_router(...)` 줄들(:108~) 뒤에 `register_risk(app, settings)` 1줄, `lifespan`(:43) 안에 `getattr(app.state, "risk_runner", None)` 가드로 `start()`/`stop()` 각 1줄. `stop()` 은 실행 중 패널을 기다리지 않고 `rr_panels.status='running'` 을 `error(retry+1)` 로, 그 5석을 `pending` 으로 되돌린다(재기동 후 러너가 `rr_jobs.state in (queued, running)` 을 다시 집는다).

### 8.2.2 Settings(`backend/app/config.py`, 전부 기본값 있는 additive 필드)

| 필드 | 형 | 기본 | 용도 |
|---|---|---|---|
| `risk_review_enabled` | bool | `False` | 라우터 등록 스위치 |
| `risk_store_path` | str | `'data/risk_review.sqlite'` | RiskStore 파일 |
| `risk_roster_domains` | str(csv) | 15 도메인 | 로스터 도메인 |
| `risk_ecad_domains` | str(csv) | `pcb,pwr,rf,soc,passive,mem` | ecad_absent 시 deferred |
| `risk_adjacency` | str(JSON) | `''` → `docs/design-risk-review/adjacency.v1.json` | counter 석 인접 표 |
| `risk_concurrency` | int | `2` | 서버 전역 동시 패널(타깃 다를 때만) |
| `risk_daily_panel_cap` | int | `24` | 일일 패널 상한 |
| `risk_default_close_level` | Literal['C2','C3'] | `'C2'` | 기본 마감 레벨 |
| `risk_carried_days` | int | `90` | carried 유효기간 |
| `risk_panel_llm_cap` | int | `120` | 사전 예산 게이트 |
| `ra_admin_pat` | str \| None | `None` | RA 관리 REST·MCP 4종(secrets, `.env`) |
| `adh_api_key` | str \| None | `None` | AIDataHub REST import(secrets) |
| `adh_team` · `adh_group` | str \| None | `None` | 사용자 확인값 없을 때의 폴백은 없음 — None 이면 레코드 import 를 `adh=unavailable` 로 미룬다 |

게이트웨이 URL·`public_base_url`·keystore·JWT issuer 는 기존 Settings 값을 재사용하고 새 키를 만들지 않는다. 러너 루프백 주소는 `public_base_url` 이다.

### 8.2.3 REST 계약(`routes.py`, prefix `/api/risk`, 모든 핸들러 `Depends(principal_pat_or_session)`(deps.py:74), 모든 쿼리 `owner_sub=principal.subject`)
...
| `POST /targets` | `{kind: snap\|diff, ref_id, consent: true}` | `{target_key, roster_size, deferred, tier_plan, cost_estimate}` | 422 `consent≠true` · 409 이미 열린 타깃 |
| `POST /targets/{key}/jobs` | `{tier: A\|B\|C, concurrency≤2, modifiers?, user_memo?, consent?}` | `{job_id, panels_planned, llm_calls_estimate}` | 422 Tier C 는 `consent:true` 필수(§6.9 명시 승인제) · 422 `reason=pat_unavailable`(§8.2.4 폴백) · 422 `reason=tier_a_web_only`(MCP 경로가 Tier A 요청, §8.3.4) |
...
| `GET /targets/{key}/brief?tier=` | | `{panels[{panel_id, seats_json, delib_opts}], evidence E0~E9, budget{bytes, dropped}}` | |
...
| `POST /panels/{id}/complete` | `{engine: mcp\|web, conv_id?, decision_text, turns[], report_id?, events?[]}` — `events[]` 는 러너가 아닌 호출자(브라우저 세션 폴백의 프론트 PanelRunner, §8.2.4)가 SSE 를 읽으며 캡처한 압축 로그 `[{kind: 'status'\|'evidence'\|'personas', step?, tool?, source?, personas?}]`, ≤400건(초과분은 앞 400건만 받고 응답에 `events_truncated=true`), 문자열 필드 각 ≤200자 | `{parsed, findings_n, coverage_updated, attribution_rate}` — 파서는 포털 단일 구현. `events[]` 가 있으면 §6.7 7단계 정규식으로 `tool_calls_n/ok`·`used_tool`·`extra_seats` 를 계산하고, 없으면 conv_store `meta.activity`(routes.py:891-899, tool 붙은 status 만)로 `tool_calls_n` 만 복원하고 `tool_calls_ok=null`·`used_tool=null`(§6.11 분모 제외) | 409 패널이 running 이 아님 · 422 `events[]` 항목 형식 위반 |
| `GET /projects/{id}/character` · `/similar` | | 프로파일 3층 · 출처별 top-k | |
| `GET /refs/{ref}` | §0.2.1 문법 | `{ref_type, resolved, payload}` | 404 dangling |
...
`GET /meta/adapters` 의 `choices[]` 는 소스 카드 선택지(StepForge `list_projects`, DynaForge `list_sessions → list_session_files`, `list_reports`)를 읽기 도구로만 채운 것이다.

### 8.2.4 인증과 사용자 PAT 대리 발급

실측 사실. 포털에는 `_chat_user_pat(keystore, settings, principal)`(routes.py:158)이 이미 있다 — 클레임 `sub=principal.subject · email · name · groups · aud=[settings.pat_chat_audience]('mcp-gateway') · scope='api' · scopes=['chat'] · pat_name='chat-session'`, 30분 창에 맞춘 결정적 발급(같은 창이면 바이트 동일, agent-server 에이전트 캐시 때문), 만료 60분. 게이트웨이는 `heax_registry.per_user_sso.kooremapper_mcp{sso_url, client:'deliberation'}` 로 DynaForge 사용자 스코프를 위임한다(gateway_config.json 확인). gap_21 의 '사실 맵에 없음' 은 함수 부재가 아니라 기술 누락이었고, 남는 실측은 세 가지다(§9 P0 (9)).

설계.

- `adapters/base.py` 에 `issue_user_pat(keystore, settings, principal_json, audience: str) -> str` 를 둔다. `audience` 는 두 값 `settings.pat_chat_audience | 'heax'` 만 받고 그 밖은 `ValueError` 다. `_chat_user_pat` 의 클레임 구성을 그대로 따르되 `aud=[audience]`, `pat_name='risk-runner'`, `jti=f"risk-{sub}-{win}"` 만 다르다. routes.py 의 함수 본체는 건드리지 않는다(복사가 아니라 형제 함수 — 이유는 rest_proxy 용 audience 가 다르기 때문이다).
- 용도별 audience(확정, 실측 불필요). 루프백 `/api/agent/chat` 과 게이트웨이 MCP 호출 모두 `settings.pat_chat_audience` 다 — `principal_pat_or_session`(deps.py:74)이 `verify_pat(..., audience=settings.pat_chat_audience)`(deps.py:93)로 고정돼 있고 `_chat_user_pat` 도 같은 값을 `aud` 에 넣는다(routes.py:186). 게이트웨이 rest_proxy 만 **site 키와 같은 문자열**(`rest_proxy.py:117 _verify(audience=site)`) 즉 `'heax'` 다. 따라서 러너는 패널마다 PAT 2장(chat 용 `pat_chat_audience`·rest 용 `'heax'`)을 발급한다.
- 타깃 생성 시 `consent:true` 와 함께 `rr_targets.principal_json{sub, email, name, groups}` 를 저장하고 러너가 패널마다 이것으로 발급한다. 동의 문구는 화면에 고정 표시한다 — "이 타깃의 무인 패널은 내 자격으로 소스 앱을 읽습니다(읽기 전용, 60분 단명 토큰, 패널마다 재발급)".
- 폴백(P0 (9)(a) 실패 시). `POST /targets/{key}/jobs` 가 422 `reason=pat_unavailable` 을 돌려주고, 패널은 사용자가 `TargetPage` 를 열어 둔 동안 프론트 PanelRunner 가 `/api/agent/chat` SSE 를 직접 열어 순차 실행한 뒤 `POST /panels/{id}/complete {engine:'web', decision_text, turns[], report_id, events[]}` 로 되돌린다. PanelRunner 는 스트림을 읽으며 `status`(tool 유무 무관, `step`·`tool`)·`evidence`(`source`)·`personas`(키·origin) 이벤트를 `{kind, step?, tool?, source?, personas?}` 로 압축해 ≤400건 모은다 — 러너가 SSE 를 직접 보지 못하는 이 모드에서 §6.7 7단계의 좌석 귀속(`tool_calls_n`·`tool_calls_ok`·`extra_seats`)을 같은 정규식으로 재구성하는 유일한 원천이다(conv_store `meta.activity` 는 tool 붙은 status 만 담고 evidence 를 저장하지 않는다, routes.py:891-899). `events[]` 없이 보내면 `tool_calls_ok=null`·`used_tool=null` 로 저장되어 그 좌석은 C2 strong 비율 분모에서 빠진다(§6.11) — 웹 엔진 패널이 전부 `done_weak` 로 떨어져 C2 가 구조적으로 미달하는 일은 없다. 이 모드에서는 무인 배치가 없고 DynaForge 소스는 브라우저 세션이 없는 시각에 `dyna absent` 로 강등된다.

### 8.2.5 어댑터 발견과 게이트웨이 설정(코드 아닌 설정)

- `adapters/registry.py` 는 `GET {gateway}/tools-map`(gateway.py:954) 으로 도구명→앱키 맵을 받아 **도구명 집합**으로 어댑터 kind 를 바인딩한다 — §2.13.2 의 집합 그대로 mcad = `{list_parts, list_interfaces, interface_graph, project_tree, part_mesh_map, job_status}` ⊂ 앱 도구, dyna = `{inspect_file, list_session_files, report_summary, report_part_risk, report_energy_flow}`, ecad = ODB 계약 4도구. 앱 이름 문자열은 코드에 없다(헌법 P8). 발견 결과가 `GET /meta/adapters` 다.
- StepForge REST 원문(`/tree`, `/artifacts/graph/`, `/parts`)은 게이트웨이 rest_proxy 를 거친다. `HWAXMcpGateway/gateway_config.json` 에 설정 2곳을 추가한다(코드 변경 0 — `rest_proxy.py:79-200` 이 임의 site 를 처리한다).

```json
"rest": { "heax": { "base": "http://127.0.0.1:4180",
                    "inject": { "header": "Authorization", "value": "Bearer <heax_registry.token>" } } },
"portal": { "audience_ok": ["mx-white-paper", "ai-data-hub", "signalforge", "heax"] }
```

`methods` 를 적지 않으면 `inject` 가 있는 사이트는 GET/HEAD 만 허용된다(rest_proxy.py:152) — 읽기 전용이 설정으로 보장되고 P0 (4) 가 POST 405 를 검증한다. 경로는 `GET {gateway}/api/heax/apps/step_forge/api/projects/{id}/tree` 처럼 `/api/{site}/{path}` 다(파일은 §0.4.5 의 `HWAXMcpGateway/gateway_config.json`).

- rest_proxy 가 불가한 환경이면 어댑터는 `capture_mode='mcp_degraded'` 로 강등해 MCP 읽기 도구만으로 IR 을 만들고 `missing.world_transform_absent` 등 결측을 남긴다(§2). StepForge 에 `export_ir` 도구를 추가하는 안은 §10 결정 사항이다.

### 8.2.6 러너·잡의 접합점

`runner.py` 는 데몬 스레드 1개 + 세마포어 `risk_concurrency` 로 §6.7 시퀀스를 돈다. 포털 자신의 `/api/agent/chat` 을 루프백 호출하므로 기존 `agent_semaphore`(`max_concurrent_chats`)·conv_store 저장·turns 60·activity 60 캡을 그대로 통과한다 — 배치가 대화형 심의 슬롯을 최대 `risk_concurrency`(2) 만큼 점유하며 P4 통과 기준 (7) 이 이를 재는 이유다. 러너는 `delib_opts` 에 `human_note`·`continue_summary` 를 절대 싣지 않는다(`_RESCREEN` 발동 차단, 불변식 `extra_seats == ∅`). `user_memo` 는 evidence 항목으로만 들어간다.
...
`_RISK_SEAT_CONTRACT`·`RISK_SEAT_CONTRACT` 의 원천은 `docs/design-risk-review/seat-contract.v1.json` 이고 두 상수는 그 파일에서 생성한 문자열을 붙여 넣는다. 포털은 같은 JSON 을 E0c evidence 항목으로도 싣는다(이중화). `_RISK_READ_TOOLS` 확장은 chair_template 무관이라 apps 로 그 앱을 고른 다른 심의도 같은 읽기 도구를 얻는다 — 의도된 읽기 전용 확장이며 decision-table.md '기존 심의 영향' 항목에 적는다. `_RISK_KEEP_TOOLS` 는 chair 조건부라 다른 심의 영향이 0 이다.

### 8.3.2 `HWAXPortal/scripts/check_chair_parity.py`

리포 루트 `scripts/` 는 현재 없으므로 P0 에서 만든다(기존 `infra/scripts/sync-workflows.sh`(§0.4.3)는 그대로 둔다).

동작. (1) `deliberation.py` 를 `ast.parse` 해 `_CHAIR_ITEMS · _CHAIR_ADVERSARY · _RISK_SEAT_CONTRACT · doc_title` 대입문을 `ast.literal_eval` 로 평가한다(암시적 문자열 연결도 평가된다). (2) `hwax-deliberate.js` 에서 `CHAIR_ITEMS · CHAIR_ADVERSARY · RISK_SEAT_CONTRACT` 객체 리터럴의 `'risk-review'`/도메인 키 값을 백틱 템플릿 리터럴 정규식으로 뽑고 `\`` 와 `\\` 만 언이스케이프한다. 제목은 삼항식에서 `'risk-review' ? '…'` 리터럴을 뽑는다. (3) `seat-contract.v1.json` 을 읽는다. (4) 항목마다 세 원천(PY · JS · JSON — 제목·결정문·반대석은 PY · JS)의 `.encode('utf-8')` 바이트를 비교한다. (5) 종료 코드 0 동일 / 1 불일치(첫 상이 바이트 오프셋과 앞뒤 40자 출력) / 2 추출 실패. `--all` 이면 기존 7 템플릿도 보고하되 `--strict` 없이는 exit 0 이다. `backend/tests/risk/test_parity.py` 가 이 스크립트를 subprocess 로 호출한다.

### 8.3.3 L1 — 단발 심사(원장 미연동)

`hwax-deliberate` 에 `chairTemplate:'risk-review'` 만 주면 결정문 8항목 + risk_spec 펜스 + 기준선 옹호 지정석이 붙은 단발 심사가 된다. 원장·등록부는 만들지 않는다. 결과를 원장에 넣고 싶으면 사람이 `POST /panels/{id}/complete` 로 보낸다(패널이 `planned` 상태로 미리 편성돼 있어야 하며, 없으면 409). P0 통과 기준 (6) 이 이 경로다.

### 8.3.4 L2 — `HWAXPortal/infra/pipeline/hwax-risk-review.js` 오케스트레이터(P5)

- args `{targetKey, tier: 'A'|'B'|'C', panels?: 1, portalBase, pat}`. `pat` 는 환경변수 `HWAX_PAT`(기존 관행, §10 확인)에서 읽어도 된다.
- 시퀀스. (1) `GET {portalBase}/api/risk/targets/{key}/brief?tier=` 로 패널 목록(`panel_id · seats_json · delib_opts 초안 · evidence E0~E9`)을 받는다. (2) 패널마다 `workflow({scriptPath:'infra/pipeline/hwax-deliberate.js'}, {question, personas: seats_json, chairTemplate:'risk-review', modifiers, evidence, rounds: 3, saveReport: true})` 를 자식 호출한다. (3) 결과 `{decision, turns, report_id}` 를 `POST /api/risk/panels/{id}/complete {engine:'mcp', decision_text, turns, report_id}` 로 되돌린다. (4) 실패한 패널은 `complete` 를 보내지 않고 `{panel_id, error}` 만 반환한다 — 포털 러너의 재시도 규칙(§6.8)이 처리한다.
- 파싱·병합·원장·외부 반영은 포털 한 곳에서만 한다. 워크플로는 risk_spec 을 파싱하지 않는다.
- 좌석 프롬프트에 "도구를 호출하지 말고 [근거]만으로 판정하라" 를 넣고, 포털은 `engine='mcp', tool_mode='evidence_only'` 로 기록한다(§6.11). Tier A 대표 패널과 무인 배치는 웹 엔진 전용이며 워크플로가 `tier:'A'` 를 요청하면 포털이 422 `reason=tier_a_web_only` 를 돌려준다.
- `meta.whenToUse` 는 순수 리터럴이다. `.claude/workflows` 사본은 `infra/scripts/sync-workflows.sh` 로 동기한다.
- 워크플로 스크립트에서 포털 REST fetch 가 불가하면(§10) 같은 페이로드를 hwax-risk 의 `risk_submit_panel_result` 로 보낸다. 두 경로의 페이로드 스키마는 같다.

### 8.3.5 포털 MCP 앱 `hwax-risk`(P5, `backend/app/risk/mcp_server.py`)

- streamable_http MCP 서버를 포털 앱에 `/api/risk/mcp` 로 마운트한다(플래그 조건부). 도구 5종의 시그니처는 §0.5.2 다. 읽기 4종은 `part` 인자로 응답 크기를 제한하고(`risk_get_snapshot(part='nodes')` 는 상위 500 노드 + `truncated`), `risk_claims_for_ref(ref)` 는 `rr_claim_refs` 조인이다. 쓰기 1종 `risk_submit_panel_result` 는 `POST /panels/{id}/complete` 와 같은 함수를 부른다.
- 게이트웨이 등록은 `gateway_config.json` 최상위 백엔드 항목 `hwax-risk: {url: 'http://127.0.0.1:8723/api/risk/mcp', transport: 'streamable_http', headers: {Authorization: 'Bearer <포털 서비스 PAT>'}}` 이다(reportarchive·signalforge 항목과 같은 모양). 게이트웨이가 `/tools-map` 에 이 5도구를 노출하면 `_RISK_READ_TOOLS['hwax-risk']` 로 다른 심의 좌석(예 sim-plan)이 apps 에 `hwax-risk` 를 넣어 등록부를 자유조회한다.
- 호출자 신원. 게이트웨이는 백엔드에 호출자 이메일을 `X-HWAX-User` 로 실어 보내고 PAT 자체는 넘기지 않는다(현 구조). hwax-risk 는 이 이메일을 `rr_targets.principal_json.email` 과 대조해 `owner_sub` 를 해석하고, 해석 실패면 `visibility=org` 행만 돌려주며 `reason='caller_unresolved'` 를 응답에 남긴다. 기본 visibility 가 private 이므로 P6 §10 결정 전에는 사실상 '자기 타깃만' 이다. 이 해석 경로는 P5 착수 시 실측한다.
...
| 원장·등록부·성격 저장 | 포털 러너 → `narrative.py` | 포털 `/panels/{id}/complete` → 같은 `narrative.py` |
| tool_mode | `tools` | `evidence_only`(C2 strong 비율 제외) |
| 제목 | doc_title '리스크 심사 보고서' | 삼항 '리스크 심사 보고서' |

### 8.3.7 decision-table.md 갱신 항목

Job 표 8행째(`risk-review` · 판단 그룹 · 트리거 `/심의 ` · chair `risk-review`) · delib_opts 계약(`personas[].origin ∈ primary|counter` 통과, `human_note·continue_summary` 러너 미탑재) · 기존 심의 영향(`_RISK_READ_TOOLS` 앱 조건부 읽기 확장, `_RISK_KEEP_TOOLS` chair 조건부) · 파리티 표(§8.3.6) · 반영 절차(§8.4.4).

## 8.4 무손상 원칙과 변경 목록

원칙 하나 — 기존 동작은 `risk_review_enabled=false` 이거나 `chair_template≠'risk-review'` 이거나 `apps` 에 소스 앱이 없을 때 1바이트도 달라지지 않는다. 그래서 모든 기존 파일 변경은 dict 항목 추가·조건부 1줄·Record 키 추가·플래그 조건부 등록으로만 한다.

### 8.4.1 기존 파일 변경(전부 additive)
...
| `HWAXPortal/backend/app/main.py` | import 1 · `register_risk(app, settings)` 1 · lifespan start/stop 2 | 4 |
| `HWAXPortal/backend/app/config.py` | §8.2.2 필드 14개(기본값 있음) | 14 |
...
| `frontend/src/App.tsx` | 라우트 객체 4 | ≈ 40 |
| `frontend/src/components/layout/AppHeader.tsx` | NavLink 1(플래그 조건부 표시) | ≈ 5 |
| `HWAXAgentServer/tools/delib_metrics.py` | 리스크 지표 7종 함수 추가(기존 지표 함수 무수정, §0.4.3) | 함수 7 |
| `docs/deliberation-quality/method-menu/decision-table.md` | §8.3.7 항목 | 문서 |
| `HWAXMcpGateway/gateway_config.json` | `rest.heax` · `portal.audience_ok` 값 1 · (P5) `hwax-risk` 백엔드 항목 | 설정 |

### 8.4.2 건드리지 않을 것

`_deliberation_stream` 라운드 루프 본체 · `_FREE_ALLOW` 전역 접두사 목록(:1233) · `_MATERIAL_TOOLS`(:1251) · `_restore_role`(:1360) 본체 · `_RESCREEN` 기본값(:307, 러너가 human_note·continue_summary 를 싣지 않으므로 발동하지 않는다 — plan_5 의 `rescreen` 옵션 신설은 채택하지 않는다) · `_chat_user_pat`(routes.py:158) 본체 · `principal_pat_or_session`(deps.py:74) · `agent_semaphore` · 기존 8 chair_template·5 modifier 문자열 · delib_opts 기존 키 의미 · SSE 이벤트 스키마(§0.2.3 은 파싱 규약이지 스키마 변경이 아니다) · RA 블록 구조·save_conversation 스키마 · conv_store/token_store 스키마(kind 값 추가만) · 기존 라우트 6개(`/`, `/login`, `/deliberate`, `/apps`, `/tokens`, `/launch/:systemId`) · ChatContext 필터 로직 · MCP 도구명 · delibTaxonomy 기존 7 Job 객체 텍스트 · StepForge/DynaForge/RA/AIDataHub 코드(RA 는 관리 REST 부트스트랩 1회 + MCP 인스턴스 쓰기 4종만, AIDataHub 는 `create_doc_type`·`create_agent`·REST import 만, StepForge/DynaForge 는 읽기 도구·REST GET 만) · `set_interface/confirm_interfaces/run_job/run_operation/remesh_parts/upload_*` 자동 실행 금지 · `bind_records_to_agent`·`patch_agent` 호출 금지 · 기존 sqlite 파일(conversations·tokens) 무접촉.

### 8.4.3 회귀 검사(불변 조건과 검사 방법)

| 불변 조건 | 검사 |
|---|---|
| `risk_review_enabled=false` 에서 `/api/risk/*` 404, `/api/agent/*` 응답 동일 | pytest 스모크(플래그 양쪽) |
...
| 게이트웨이 도구 수 드리프트 0(P5 전) / +5(P5 후) | `/tools-map` len 비교 |
...
### 8.4.4 반영 절차(운영, 사용자 몫)

순서대로. (1) agent-server 재기동(`deliberation.py` 상수·`_g` 조립 조건·`_narrow`·:2044 계약 접미). (2) `infra/scripts/sync-workflows.sh`(hwax-deliberate.js 변경, P5 부터 hwax-risk-review.js). (3) erag 재색인(whenToUse 변경). (4) 포털 `.env` 에 `RISK_REVIEW_ENABLED=true` 와 secrets(`RA_ADMIN_PAT`, `ADH_API_KEY`) → 포털 재기동(`data/risk_review.sqlite` 생성). (5) 게이트웨이 `gateway_config.json` `rest.heax`·`audience_ok` 반영 → 게이트웨이 재기동(restart 스크립트의 `-sTCP:LISTEN` 주의, 메모리 restart-kill-port-hazard). (6) `bootstrap_ra_ontology.py --apply`(관리자 PAT, 승인 후). (7) `bootstrap_adh.py`(doc_type 3종·risk-review-memory). (8) P5 에서 게이트웨이에 `hwax-risk` 항목 추가 후 재기동. dev 박스에서 `deploy-all` 류를 돌리지 않는다(메모리 deploy-all-resets-dev-repo).

# §9 단계 계획

## 9.0 읽는 법

단계마다 목표 · 산출물 · 의존 · 수치 통과 기준 · 리스크와 완화 · "이 단계만으로 얻는 것" 을 적는다. 각 단계는 그 자체로 사용자가 무언가를 등록·조회·심사할 수 있는 완결 기능이어야 하고, 통과 기준은 전부 코드나 스크립트로 판정 가능한 숫자다. 1차 계획(doc_20 §9)을 정본으로 보존하되 gap_21 이 확인한 것을 다음처럼 고쳤다 — parse_risk_spec 을 P0 산출물로 당김(공백 7), C1 조건에서 RA 절을 빼고 `external_sync` 로 표기(공백 7), P0 (9) 에 `_chat_user_pat` 실측 3항(공백 9), `_RISK_SEAT_CONTRACT`·`_RISK_KEEP_TOOLS` 를 P0 확정 변경으로(공백 3·4), P3 에 SSE 귀속 성공률(공백 5), P4 에 미착석 배지·`risk_default_close_level`(공백 6), P5 에 계보 없는 과제 회수·E7 슬롯(공백 2·8), 비용 표의 toulmin 재시도 줄 삭제·사전 예산 게이트(공백 10), `extra_seats == ∅`(공백 11). 문서 디렉터리는 `docs/design-risk-review/` 다.

의존 사슬은 P0 → P1 → P2 → P3 → P4 → P5 → P6 → P7 이 기본이고, 병행 가능한 것은 §9.9 에 있다.

## 9.1 P0 부트스트랩 — 계약·엔진 손잡이·저장소 준비

목표. 저장소 3층의 계약·엔진 dict 항목·게이트웨이 설정·secrets 를 준비해 이후 단계가 계약 위에서만 움직이게 하고, 핸드오프 카드에서 단발 리스크 심사(L1)가 즉시 돌아가게 한다.

산출물.
- 문서 `docs/design-risk-review/{plan.md, checklist.md, context-notes.md, taxonomy.v1.json, character-vocab.v1.json, character-seed-rules.v1.json, adjacency.v1.json, rules-seed.v1.json, seat-contract.v1.json, odb-adapter-contract.md}`.
- 스키마 `backend/app/risk/schemas/{rr_ir, rr_state, rr_diff, risk_spec, seat_opinion}.v1.json` + 유효/무효 픽스처 각 ≥2.
- `backend/app/risk/narrative.py` v0 = `parse_risk_spec`(parseSimSpec 복제, 펜스 우선 → 마지막 균형 중괄호 → 실패 null 비치명) + `risk_spec.v1.json` 검증. cites 해석·seat_opinion 추출·prior_evidence 는 P1·P3 다.
- `backend/app/risk/taxonomy.py` + `__init__.py register()` + `routes.py` 골격(`GET /meta/taxonomy`, `/meta/adapters`) + `risk_store.py` 빈 스토어 + Settings 14 필드.
- `backend/scripts/bootstrap_ra_ontology.py`(6축 · 속성 · 12관계, dry-run 기본, 멱등, §0.3.2 전표를 스크립트 상단 표로 포함 — `revision_of{project→project, acyclic}` 포함).
- `backend/scripts/bootstrap_adh.py`(doc_type `risk_review_opinion · risk_review_panel · project_character`, 의사 에이전트 `risk-review-memory`).
- `deliberation.py` 항목 — `_CHAIR_ITEMS · _CHAIR_ADVERSARY · doc_title · _RISK_SEAT_CONTRACT(+:2044 직후 p["role"] 접미 3줄) · _RISK_READ_TOOLS · _RISK_KEEP_TOOLS(+자유조회 조립식 _g 조건 확장 ≈3줄 :1897-1898 + _amap 상향 1줄 + _narrow keep or 1줄 :1916-1918) · _resolve_opts origin 1줄(:434-435)`. `hwax-deliberate.js` 항목 — `CHAIR_ITEMS · CHAIR_ADVERSARY · 제목 삼항 · RISK_SEAT_CONTRACT · whenToUse`. `HWAXPortal/scripts/check_chair_parity.py`.
- `routes.py` ConvCreate kind 값. `delibTaxonomy.ts`·`conversations.api.ts` 값. `App.tsx` 라우트 4 + `RiskHomePage` 플레이스홀더(플래그 안내만) + `AppHeader` NavLink(조건부).
- 게이트웨이 `gateway_config.json` `rest.heax`·`audience_ok`.
- `backend/tests/risk/` 신설(pytest 설정 포함) — 스키마 라운드트립 · 파서 · parity · `_g` 조립·`_narrow` · 플래그 스모크.
- `HWAXAgentServer/tools/delib_metrics.py` 에 `risk_spec 파싱 성공률` 1종(나머지 6종은 P4).

의존. 없음.

통과 기준.
1. `check_chair_parity.py` 가 결정문·반대석(key/label/role)·좌석 계약 16 문자열(15 도메인 + `_common`, §6.5.4)·제목 을 PY/JS/JSON 바이트 동일로 보고, exit 0.
2. RA `list_object_types` 에 신규 6축·12관계가 보이고, 스크립트 2회 실행 시 두 번째는 생성 0건, 기존 15축·17관계 무변경(dry-run 출력과 실행 후 조회 diff 0).
3. AIDataHub `list_doc_types` 에 3종, `list_agents` 에 `risk-review-memory` 존재. 재실행 시 생성 0.
4. `issue_user_pat(aud='heax')` 로 `GET {gateway}/api/heax/apps/step_forge/api/projects/{id}/tree` 200, 같은 경로 `POST` 405, aud 가 다른 PAT 는 401.
...
9. PAT 실측 기록(context-notes.md) — `_chat_user_pat`(routes.py:158) 클레임·창·만료 표를 옮겨 적고 세 항목을 실측한다. (a) `rr_targets.principal_json` 으로 복원한 Principal 로 `issue_user_pat(audience=settings.pat_chat_audience)` 가 발급한 PAT 로 포털 자신의 `POST /api/agent/chat` 이 200 인가(aud 값은 deps.py:93 으로 이미 확정 — 통과 여부만 본다). (b) 그 PAT 로 agent-server → 게이트웨이 → DynaForge `per_user_sso` 가 사용자 세션 파일을 여는가(list_sessions 로 확인). (c) rest_proxy 는 `aud=site` 가 필요함을 확인(P0 (4) 와 같은 호출). (a) 가 실패하면 §8.2.4 폴백(`events[]` 경유 귀속)을 확정하고 P2 (8)·P3 (6) 의 기대값을 폴백 기준으로 바꾼다.
10. pytest `backend/tests/risk/` 스키마 라운드트립 5종 통과, 무효 픽스처 거부 ≥10건.
11. 프론트 `tsc -b && vite build` 통과, playwright 스모크 — `/deliberate` Job 카드 8개, 기존 7개 카드 텍스트 불변, `/risk-review` 가 플래그 off 에서 안내 화면.
12. `risk_review_enabled=false` 에서 `/api/risk/*` 404, `/api/agent/conversations` 생성·조회 응답이 변경 전 스냅샷과 동일.

리스크와 완화. RA 관리자 PAT·AIDataHub API 키 발급 지연 → (2)(3) 을 보류하고 P1~P4 를 sqlite 만으로 진행, 외부 반영은 `external_sync=unavailable` 로 표기. 게이트웨이 설정 권한 없음 → (4) 보류·어댑터 `mcp_degraded`. GLM 펜스 준수율 <95% → 균형 중괄호 폴백이 (6) 을 채우는지 기록.

이 단계만으로 얻는 것. 챗 핸드오프 카드 '리스크 심사' 와 MCP `hwax-deliberate chairTemplate:'risk-review'` 로 evidence 기반 단발 심사(결정문 8항목 + risk_spec + 기준선 옹호 지정석)가 즉시 돌아간다.
...
산출물. `risk_store.py`(P1 표 — rr_projects · rr_sources · rr_snapshots · rr_snapshot_calls · rr_ir_nodes · rr_ir_edges · rr_part_keys · rr_dim_vocab · rr_dim_defs · rr_states · rr_id_map), `adapters/{base, registry, mcad, ecad_stub}.py`, `ir_builder.py`, `sameas.py` v0(ckey 부여 · rr_part_keys candidate 등록 · 원장 재적용 골격, 사다리는 P2), `state.py`, `render.py`, `routes.py`(projects · sources · snapshots · dims · refs · rule_hits · meta/adapters choices), rr_rules 시드 6종 로드(`rules-seed.v1.json`), `RiskHomePage · ProjectPage · SnapshotPage`, `narrative.prior_evidence` v0(E0 · E1 · E9) + '단발 심사 열기'(startHandoff 재사용), `ra_client` v0(project · design_snapshot upsert, `revision_of` 링크, 비치명), 골든 `tests/risk/golden/sif-e2e.ir.json`.

의존. P0.

통과 기준.
1. sif-e2e(재파싱·재검출 후) 리프 노드 3·엣지 2 가 REST graph.json 과 일치, 2회 추출 ir_hash 동일, 같은 `(project_id, ir_hash)` 재요청 시 기존 snapshot_id 반환.
2. REST 원문만 읽고 StepForge sqlite mtime 불변(전후 stat 비교).
...
11. ruff·pytest 통과, 기존 `/deliberate` 스모크 통과, `/api/agent/*` 응답 동일.

리스크와 완화. 기존 프로젝트 volume/material null → parse·detect 재실행은 사용자가 StepForge 에서 실행(화면 안내·링크). REST 경로 불가 → `mcp_degraded` 강등 + `missing.world_transform_absent`. 골든 프로젝트 부재 → §10 (15).

이 단계만으로 얻는 것. 과제 등록 · 스냅샷 현황판 · 게이트 · 성격 씨앗 · 규칙 히트 · 단발 심사 브리프(E0 · E1 · E9).

## 9.3 P2 Dyna 어댑터·same-as·원장·3층 diff·summary_text

목표. MCAD+Dyna 를 한 IR 로 합치고 두 스냅샷의 구조/파라메트릭/의미 diff 와 결론 없는 요약을 낸다.

산출물. `adapters/dyna.py`(`inspect_file`·`report_*` 읽기, 사용자 PAT, 브리지 검증, `_group_of` 재구현), `sameas.py`(사다리 7단 · 헝가리안 · 원장 재적용 · ckey), `rr_sameas · rr_iface_ledger` 재적용·`ledger_needs_review` 규칙, `diff.py`(3층 · 임계 · comparability · rollup · 의미 이벤트 · 판단어 린터), `rr_diffs · rr_diff_events`, `routes`(sameas · iface-ledger · diffs), `SameAsResolver · GateBanner · DiffView · ComparePage`, `prior_evidence` v1(E2 · E3 · E4), 합성 픽스처 6종(추가/삭제/kind_changed/두께/재료/이동), `ra_client` design_diff + `diff_of(role)`.
...
7. 노드 500·엣지 2000 에서 diff <5 s.
8. 게이트웨이 서비스 계정으로 DynaForge private 세션을 열지 않음(사용자 PAT 없으면 `missing.dyna_absent=true` 기록, 예외 0).
...
리스크와 완화. 사용자 PAT 전달 배선(P0 (9) 결과 의존). DynaForge 실데이터(세션 0·리포트 0) → 사용자가 세션·리포트를 먼저 만든다(§10 (15)). fuzzy pending 누적 → G2 가 의미층만 막는 정상 동작.

이 단계만으로 얻는 것. 두 과제 비교 화면(3층 diff · before/after 그래프 · 결론 없는 요약)과 pair 단발 심사(E0~E4).
...
산출물. `narrative.py` 완성(cites 해석 · dangling · evidence_grade 자동 산출 · seat_opinion 추출 · E0c 계약 항목 · prior_evidence v2 E0~E4+E9), `runner.py` 단일 패널 모드(루프백 · `issue_user_pat` · SSE 캡처 §0.2.3 · 사전 예산 게이트 · 커버리지 갱신), `registry.py` 병합기(cluster_key · contested · verdict 후보), `character.py`(L2 panel 진술 저장), `ra_client`(assessment · risk_finding · exhibits · 보고서 tags 병합 — get_report 후 병합), `adh_client`(REST UPSERT `risk_review_opinion · risk_review_panel`), `rr_targets · rr_panels · rr_seat_opinions · rr_findings · rr_registry · rr_claim_refs · rr_character` 표, `TargetPage` 최소(패널 1건 실행 · 결과 표 · 등록부 · 대화 열기), decision-table.md 갱신, SSE 픽스처 스트림 3종(정상 · 도구 0 · 오류).
...
5. 패널 대화가 `kind='risk-review'` 로 저장되고 `/deliberate` 목록에 0건 노출, 제목 접두 `[리스크심사]`.
6. AIDataHub 재실행 시 레코드 중복 0(`_external_id` UPSERT), RA assessment `create_object` 2회 호출 시 `created:false`, 의견 레코드 `agents` 배열에 실 전문가 키 0건.
7. 패널 시작 70분 뒤 두 번째 패널에서도 DynaForge 도구 호출 성공(PAT 재발급·에이전트 캐시, P0 (9)(a) 성공 시) 또는 폴백 모드에서 dyna absent 표기.
...
9. MCP `hwax-deliberate chairTemplate:'risk-review'` 단발 결과를 `POST /panels/{id}/complete {engine:'mcp'}` 로 보내면 원장에 `tool_mode=evidence_only` 로 반영, 좌석은 `done`·`tool_calls_n=null`.
10. `evidence_profile` 헤더와 seat_opinion 합계 불일치 시 `quality.flag=header_mismatch` 가 붙는 픽스처 1건.
11. 기존 `/deliberate`·핸드오프 회귀 0.
...
리스크와 완화. GLM 펜스 준수율 <95% → 균형 중괄호 폴백, 실패는 `risk_spec_parsed=false` 로 남기고 재실행 후보(자동 재실행 없음). RA 관리자 PAT 부재 → `external_sync.ra=unavailable`, C 레벨 판정과 무관.

이 단계만으로 얻는 것. 등록부 · 성격 서술 · 좌석 의견이 3층에 저장되는 완결 심사 1건과 MCP 결과의 원장 반영.

## 9.5 P4 커버리지 원장·편성·배치 러너·완결 판정·통합 보고서

목표. 로스터를 Tier A/B/C 로 소진하는 배치를 원장 상태기계 위에서 돌리고 레벨별 통합 보고서를 낸다.

산출물. `planner.py`(Tier · 라운드로빈 · 인접 표 · deferred · carried · 결정론), `rr_roster · rr_coverage`(부분 유니크 인덱스 `rr_cov_active`) · `rr_jobs`, runner 배치 모드(세마포어 · pause/resume/cancel · 수확 체감 정지 · 일일 상한 · 재기동 복구), 완결 판정 C1~C3(`registry.py`), 통합 보고서 조립기(RA `deliberation` 템플릿 v1/v2/v3), `TargetPage` 완성(`CoverageHeatmap · PanelRunner · verdict 확정 · 미착석 배지`), `delib_metrics.py` 지표 6종 추가(도구 사용률 · IR 인용률 · 근거등급 분포 · out_of_range · 반대석 기각률 · 클러스터 중복률), 상태기계 불변식 테스트.

의존. P3.

통과 기준.
1. Tier A 3패널 완주 후 15 도메인 종결 ≥1 → `level='C1'`, 통합 보고서 v1 생성. **RA 미가용 상태(`ra_admin_pat` 비움)에서도 C1 도달**하고 `external_sync.ra='unavailable'` 로 표기(RA 는 C 조건이 아니다).
...
12. 재기동 후 `running` 패널이 `error(retry+1)`·5석 `pending` 으로 복구되고 잡이 이어진다.

리스크와 완화. 야간 GPU 점유 · dev 박스 OOM(메모리 dev-box-oom-pattern) → 일일 상한 · concurrency 1 옵션. xd 편중 → Tier C 규칙(xd 최대 3석 · 비-xd ≥2석).
...
산출물. `prior_evidence` 완성(E5~E8 · 계면 별칭 · kNN · hybrid_search · agent_search(risk-review-memory) · 예산표), `rr_delta_priors · rr_iface_alias`, `rr_part_keys` merge/rename UI(ProjectPage), `mcp_server.py`(hwax-risk 5도구) + 게이트웨이 항목, `_RISK_READ_TOOLS['hwax-risk']`, `hwax-risk-review.js` + sync, 성격 프로파일 승격 UI(seed → panel → confirmed, AIDataHub `project_character` UPSERT), `RecallPreview`, `GET /precedents`, `GET /projects/{id}/similar` 4경로.
...
6. sim-plan 심의 좌석이 apps 에 `hwax-risk` 를 넣고 `risk_get_registry` 자유조회 성공, `/tools-map` 도구 수 = 카탈로그 len + 5.
7. MCP 워크플로 1패널이 `/panels/{id}/complete` 경유로 원장 `done`, `engine=mcp` 배지, `tier:'A'` 요청은 422.
8. prior_evidence 문장 판단어 0, `[선례 없음 — n=0]` 부정 증거 항목이 첫 사례에 실린다.
9. hwax-risk 호출자 신원 해석(X-HWAX-User → principal_json.email)이 자기 타깃을 돌려주고 타인 타깃은 `caller_unresolved`/빈 결과.

리스크와 완화. e5 코사인 절대값 편향 → 상대 순위만(메모리 e5-embedder-baseline). 포털 MCP 앱 등록 절차(게이트웨이 백엔드 항목) 권한 확인. 워크플로 REST fetch 불가 → `risk_submit_panel_result`.

이 단계만으로 얻는 것. 배치가 쌓일수록 브리프가 두터워지는 폐루프와 다른 connectivity 회수.

## 9.7 P6 학습 루프 — 라벨·지표·패턴 승격·규칙 백테스트·조직 공유 정책

목표. 실측(incident/test_run/후속 리포트)으로 finding 을 검증해 규칙 후보를 승격하고 학습 곡선을 계측한다.

산출물. `rr_labels · rr_patterns · rr_rules(active 승격) · rr_metrics · rr_curation_queue`, 라벨 동기 잡(RA incident · test_run · DynaForge 후속 리포트 · SignalForge VOC · 수동), 조건 DSL 평가기 · 백테스트(시간순 홀드아웃), 지표 대시보드('품질' 카드, dataviz 규약) · '루프 작동' 배지, `risk_pattern_card` doc_type, `visibility` 정책(org 토글, §10 결정 후), RA 'risk-review' typed 템플릿 등록 절차서(관리자).
...
리스크와 완화. 라벨 유입 속도 → 지표가 n<5 로 오래 머묾(정직하게 표기). 공유 정책은 사용자 결정.

이 단계만으로 얻는 것. 검증된 선례·규칙과 루프 작동 여부의 계측.

## 9.8 P7 ODB 실연동·예측기 승격(조건부)

목표. ODB hub 가 계약을 이행하면 ECAD 를 IR 1.1 로 편입하고, 라벨이 충족되면 예측기 앱 계약을 이행한다.

산출물. `adapters/ecad.py`(계약 4도구 `odb_get_board · odb_list_components · odb_list_nets · odb_get_stackup`), ir_version 1.1, refdes↔파트 사전 UI, `ecad.*` 의미 이벤트, SedInput 매핑 어댑터(`predict_sed` 를 좌석 도구로 — `_RISK_KEEP_TOOLS` 에 이미 있음), 예측기 MCP 앱 계획서(thermal_shock 수명주기 복제, 별도 리포).
...
|---|---|---|---|---|
| P0 | — | 문서·스키마·parity 는 즉시, RA/AIDataHub 부트스트랩은 (1)(10) 승인 후 | 단발 3회 | (1)(2)(3)(10) |
| P1 | P0 | 프론트 3화면은 P0 (11) 뒤 바로 | 0 | (15)(14) |
| P2 | P1 | dyna 어댑터는 P0 (9) 결과 확정 후 | 0 | (4)(15) |
| P3 | P0·P2 | narrative/registry 는 P2 와 병행(픽스처 diff 로) | 1패널 ≈50~95회 | (3) |
| P4 | P3 | planner 결정론 테스트는 P3 와 병행 | Tier A 3패널, Tier B +14~20(§6.4.1 로스터 실측 산술) | (5)(6)(7) |
| P5 | P4 | hwax-risk MCP 앱·hwax-risk-review.js 는 P3 뒤 착수 가능 | 후임 타깃 1건 | (8)(13)(16) |
| P6 | P5 | — | 0(라벨·지표) | (11)(12) |
| P7 | P6 / P2+(9) | ECAD 어댑터는 계약 합의 즉시 | — | (9) |

비용 표에서 'toulmin 켜면 parse_retries 하한 2' 줄은 삭제했다 — 하한 2 는 `rebut_quote` 조건이고 그것은 `search_sources` 가 있을 때만 켜진다. 120 초과는 사후 재실행이 아니라 사전 편성(rounds 2)과 사후 `over_budget` 표기다.

# §10 열린 질문·결정 필요

사용자가 답해야 하는 것만 모았다. 항목마다 질문 · 답이 없을 때 계획이 가정하는 기본값 · 영향 단계 · 결정이 필요한 시점을 적는다. 번호는 §9.9 표가 참조한다.

## 10.1 승인·보안

| # | 질문 | 기본값(미결 시) | 영향 | 시점 |
|---|---|---|---|---|
| 1 | RA 시스템관리자 PAT 를 포털 서버 secrets(`ra_admin_pat`)에 서비스 계정으로 보관하는 것을 승인하는가. RA 코드는 무수정이고 관리 REST 부트스트랩 1회 + MCP 인스턴스 쓰기 4종(`create_object · update_object · add_object_alias · link_objects`)에만 쓴다 | 미승인 — P0 (2) 보류, P1~P5 는 sqlite 만으로 진행, `external_sync.ra='unavailable'`. C 레벨 판정은 영향 없음 | P0 (2) · P3 (6) · P4 (1) | P0 착수 전 |
| 2 | 게이트웨이 `gateway_config.json` 에 `rest.heax`(heax 서비스 토큰 inject, GET/HEAD 만)와 `portal.audience_ok` 값을 추가하는 설정 변경 권한을 주는가. 코드 변경은 없다(rest_proxy.py:79-200 확인). 불가하면 StepForge 에 `export_ir` 읽기 도구 1개 추가(코드 변경 · SIF 재빌드)를 허용할 것인가 | 설정 변경 승인 가정. 둘 다 불가면 어댑터 `mcp_degraded`(world 좌표 결측) | P0 (4) · P1 (2)(3) | P0 |
| 3 | 배치 러너가 타깃 owner 의 `principal_json` 으로 사용자 PAT 를 패널마다 대리 발급하는 것(`issue_user_pat`, 60분 단명, 읽기 전용 경로, 타깃 생성 시 `consent:true`)을 승인하는가. 동의 문구는 §8.2.4 다 | 승인 가정. 미승인이면 §8.2.4 폴백(브라우저 세션에서만 패널 실행, 무인 배치 없음, dyna absent 강등) | P0 (9) · P3 (7) · P4 전체 | P0 |
| 4 | DynaForge 사용자 스코프 — 게이트웨이 `per_user_sso.kooremapper_mcp` 가 러너 발급 PAT 의 email 클레임으로도 사용자 세션을 여는지 P0 (9)(b) 에서 실측한다. 열리지 않으면 dyna 소스를 사용자 브라우저 세션에서만 동결할 것인가, DynaForge 세션 visibility 를 department 로 넓히고 SSO 소속값을 채울 것인가 | 실측 결과에 따름. 미결이면 브라우저 세션 동결 | P2 (8) · P3 (7) | P0 (9) 직후 |
| 16 | hwax-risk MCP 앱의 호출자 신원 — 게이트웨이가 백엔드에 넘기는 `X-HWAX-User` 이메일을 `rr_targets.principal_json.email` 과 대조하는 §8.3.5 방식을 승인하는가, 아니면 게이트웨이가 호출자 PAT 를 통과시키는 설정이 있는가 | §8.3.5 방식, 해석 실패는 `caller_unresolved` | P5 (9) | P5 착수 |
...
| # | 질문 | 기본값 | 영향 | 시점 |
|---|---|---|---|---|
| 10 | AIDataHub 의견 레코드의 team/group 값(과제 등록 화면에서 사용자가 1회 입력 · 자동 채움 금지)과 열람 범위, 의사 에이전트 `risk-review-memory` 를 `create_agent` 로 만드는 것의 승인 | 승인 가정. 미승인이면 `external_sync.adh='unavailable'`, E6·E7 은 포털 sqlite 만으로 조립(agent_search 경로 없음) | P0 (3) · P3 (6) · P5 (3) | P0 |
...
| 6c | RA · AIDataHub 외부 반영 재시도 큐의 보관 기간(무기한인가, N일 후 `unavailable` 확정인가) | 30일 후 `unavailable` 확정, 수동 재시도 버튼 | P3 · P4 | P3 |

## 10.7 워크플로·MCP

| # | 질문 | 기본값 | 영향 | 시점 |
|---|---|---|---|---|
| 13 | Workflow 스크립트(`hwax-risk-review.js`)에서 포털 REST fetch 가 허용되는가, PAT 는 어느 환경변수(`HWAX_PAT` 관행)로 받는가. 불가하면 `hwax-risk` 의 `risk_submit_panel_result` 로만 원장을 잇는다 | REST fetch 가능 가정 | P5 (7) | P5 착수 |
...
| 13b | 게이트웨이 백엔드 항목에 `hwax-risk`(포털 `/api/risk/mcp`)를 등록하는 것과 그때 쓰는 포털 서비스 PAT 발급 | 승인 가정 | P5 (6) | P5 |

번호가 겹치는 보조 항목(8a · 12a · 14a 등)은 같은 결정 묶음의 하위 질문이다. 위 표의 어떤 항목도 답이 없다고 P0~P4 가 멈추지 않는다 — 기본값 열이 그 상태에서 계획이 취하는 동작이다. 답이 오면 `docs/design-risk-review/context-notes.md` 에 결정과 근거를 적고 Settings 기본값·문서 임계값을 그에 맞춘다.
