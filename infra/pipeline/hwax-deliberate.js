...
//                       'sim-plan'(해석 계획서 10항목 — 식별성 판정과 한계를 강제). 시뮬레이션 심의
//                       (hwax-sim-deliberate.js)가 1단/2단에서 각각 지정한다.
...
const decision = await agent(
  ...
  `[${SEAT_NOTE}]\n\n` +
  `산출: ## ${CHAIR_TEMPLATE === 'sim-plan' ? '해석 계획서' : '의사결정문'} — (0) 참여 도메인과 커버리지 한계 — 위 좌석 구성을 한 문단으로 기록하고, 이 문제에 관련되나 착석하지 않은 인접 도메인이 있으면 명시하라(없으면 없다고 쓰라), ` +
